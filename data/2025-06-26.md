...
Motivation: 低资源语言缺乏平行语料库，而现有LLMs在机器翻译上表现不如专用系统。

Method: 提出CycleDistill方法，通过零/少量示例生成合成平行语料库，并迭代优化模型。

Result: 在三种印度语言上，首轮迭代平均提升20-30 chrF点，且利用softmax激活进一步改善质量。

Conclusion: CycleDistill无需大量平行语料库，即可显著提升低资源语言的机器翻译质量。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）在知识密集型推理任务中表现不佳，因缺乏结构化上下文和多跳信息。传统RAG方法未能充分捕捉知识图谱中的关系结构。

Method: 提出Inference-Scaled GraphRAG框架，结合顺序扩展（深度链式图遍历）和并行扩展（多数投票采样轨迹），在推理-执行循环中增强图推理。

Result: 在GRBench基准测试中，该方法显著提升多跳问答性能，优于传统GraphRAG和图遍历基线。

Conclusion: 推理时间扩展是一种实用且与架构无关的解决方案，可提升LLM在结构化知识推理中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有API代理工具集过于简化、无法应对现实世界API复杂性的问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有时空数据挖掘模型局限于单一任务，缺乏多任务推理和复杂长推理能力，限制了其在现实决策场景中的应用。

Method: STReason利用上下文学习将复杂自然语言查询分解为模块化、可解释的程序，并系统执行以生成解决方案和详细解释。

Result: 实验表明，STReason在所有指标上显著优于先进的LLM基线，尤其在复杂时空推理场景中表现优异。人类评估进一步验证了其可信度和实用性。
...
<details>
  <summary>Details</summary>
Motivation: 当前代码检索器过于依赖表面文本特征（如文档字符串、标识符名称）且对文档化代码有强烈偏见，影响了代码生成的效果。

Method: 通过系统性地屏蔽特定特征但保留代码功能，分析代码检索行为，并提出SACL框架以增强语义信息。

Result: SACL显著提升了代码检索性能（如HumanEval上Recall@1提高12.8%），并进一步改善了代码生成（如HumanEval上Pass@1提高4.88%）。
...
<details>
  <summary>Details</summary>
Motivation: 解决分布语义与符号语义之间的鸿沟，提升语言模型的语义理解能力。

Method: 比较了三种主流自编码器架构（VAE、VQVAE、SAE），分析了它们对语义结构和可解释性的潜在几何影响。

Result: 通过组合语义视角，为语义表示学习提供了新方向，有助于弥合符号与分布语义的差距。

Conclusion: 语义表示学习为语言模型的语义理解提供了新思路，未来可进一步探索其应用潜力。
...
TL;DR: 三阶段LLM框架显著提高放射报告的阳性预测值（PPV）并降低运营成本，同时保持检测性能。


<details>
  <summary>Details</summary>
Motivation: 由于放射报告中错误率较低，现有LLM校对方法的PPV有限，需改进框架以提高效率和准确性。

Method: 采用三种LLM框架（单提示检测器、提取器加检测器、三阶段框架）对1000份放射报告进行回顾性分析，并验证于外部数据集。

Result: 三阶段框架PPV显著提升至0.159，运营成本降低42.6%，人工审核报告减少。
...
<details>
  <summary>Details</summary>
Motivation: 评估学习者高阶能力的需求增加，但传统构建反应测试的人工评分成本高且效率低。IRT虽能从不完整分数数据估计能力，但缺失分数比例增加时准确性下降。

Method: 结合自动评分技术填补缺失分数，优化IRT能力估计。

Result: 新方法显著提高了能力估计的准确性，同时大幅减少了人工评分工作量。

Conclusion: 该方法为高效评估学习者能力提供了可行解决方案，尤其适用于稀疏或异构数据场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG系统评估方法存在不足，如依赖简单词汇重叠指标或复杂多阶段流程，难以高效捕捉多维质量。

Method: 提出CCRS，包含五个指标（CC、QR、ID、AC、IR），利用单一预训练LLM作为零样本端到端评估器。

Result: CCRS在BioASQ数据集上有效区分不同RAG系统性能，如Mistral-7B优于Llama变体，且计算效率高。
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型（LRMs）通过生成冗长的思维链实现强大推理能力，但这种“过度思考”导致高延迟和成本，而准确性提升有限。

Method: AALC将验证准确性整合到强化学习的奖励中，并采用动态调度的长度惩罚，延迟长度惩罚直到达到目标性能。

Result: 实验表明，AALC在标准和非分布数学基准测试中，减少响应长度超过50%，同时保持或提高准确性。

Conclusion: AALC通过奖励策略引导LRMs生成更高效、通用的推理路径，但可能牺牲部分解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在建模特征交互时缺乏语义推理和任务适应性，而语言模型无法直接处理时间序列数据，限制了统一预测系统的发展。

Method: SEED包含四个阶段：令牌感知编码器提取补丁，投影模块对齐补丁与语言模型嵌入，语义重编程映射补丁到任务原型，冻结语言模型进行预测。

Result: 实验表明SEED在多个数据集上优于基线方法，有效填补了结构-语义建模的空白。

Conclusion: SEED通过解耦表示学习与推理，实现了数值模式与语义推理的高效对齐，推动了统一预测系统的发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有UQ方法缺乏形式化保证的问题，特别是选择性预测中的FDR控制，以提升生成文本的实用性和可靠性。

Method: 采用统计方法（如Clopper-Pearson）校准阈值，估计校准集上的经验错误率，并建立真实错误率的高概率上界。

Result: COIN在风险控制、测试时保留合格答案的能力以及有限校准数据下的预测效率方面表现出色。

Conclusion: COIN具有扩展性和适应性，适用于多种应用场景，并能通过替代上界构建和UQ策略进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型（LLM）应用广泛，但在主观任务（如情感识别）中实现高准确性仍具挑战性。

Method: 提出随机和增强示例检索策略，并分析对话上下文对CER的影响，实验基于IEMOCAP、MELD和EmoryNLP数据集。

Result: 增强示例检索在所有数据集中表现最优，表明检索连贯目标示例并通过改写增强的重要性。

Conclusion: 增强示例检索是提升CER性能的有效方法，强调了高质量示例和上下文优化的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 探讨句子嵌入模型在捕捉语义相似性和下游任务（如翻译评估）中的表现差异，揭示语义属性与任务性能的复杂关系。

Method: 使用Costra数据集和STS基准进行内在评估，通过COMET指标进行外在评估。

Result: 内在语义测试表现优异的模型在下游任务中不一定更好，而某些看似平滑的嵌入模型通过微调能取得优异结果。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法通过聚合标注者观点生成单一真实标签，但可能忽视少数观点，尤其是在主观任务中。本研究旨在解决这一问题。

Method: 采用多视角软标签方法，并在多个主观文本分类任务（如仇恨言论、讽刺、辱骂语言和立场检测）中进行实验。

Result: 多视角方法在Jensen-Shannon Divergence（JSD）和F1分数上优于传统方法，但在讽刺和立场检测任务中置信度较低。

Conclusion: 多视角方法能更好地反映人类标注的多样性，但需进一步解决高主观性任务中的置信度问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLMs在复杂逻辑推理任务中表现不佳，主要依赖统计关系而缺乏结构化知识表示。

Method: 将非结构化数据转换为结构化格式，通过监督微调（SFT）训练LLMs，并引入GRPO算法（MAX-Flow和LCS）优化推理能力。

Result: 实验表明，该方法在DeepSeek-R1-Distill-Qwen-1.5B模型上实现了简洁推理、鲁棒性能和优化兼容性。

Conclusion: 结构化推理的集成显著提升了LLMs的推理能力，验证了其有效性。
...
Method: 采用分块方法，结合多种自监督学习模型（Wav2Vec2、HuBERT、WavLM）和分层CNN-BiLSTM框架，通过加权机制融合特征并加入分块级流利度标记。

Result: 在Avalinguo和Speechocean762数据集上，F1分数和Pearson相关系数分别提升了2.8-4.2和6.2-4.0点，优于基线方法。

Conclusion: 分块多自监督学习融合方法对流利度评估具有鲁棒性，但未来需探索其在非规则韵律方言中的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 随着媒体叙事的快速演变，研究叙事如何随时间发展变得至关重要。现有方法（如大型语言模型）在提取叙事元素时面临高成本或计算复杂性的问题。
...
Conclusion: 该方法为动态叙事分析提供了可行方案，但需进一步优化以提升区分能力。
...
<details>
  <summary>Details</summary>
Motivation: 临床文本通常因隐私限制难以获取，Biomed-Enriched提供了一个公开的大规模临床病例数据集，支持生物医学和临床NLP研究。

Method: 通过大语言模型标注400K段落，再微调小语言模型扩展到PMC-OA语料库，提取高质量子集并进行质量过滤和领域上采样。

Result: 实验表明，临床上采样提升MMLU ProfMed性能约5%，教育质量过滤提升MedQA和MedMCQA约1%，组合方法减少训练标记需求。

Conclusion: Biomed-Enriched为生物医学预训练提供了高效策略，数据集可作为有价值的资源。
...
Result: TAPS显著提升了语言模型在NLSI任务中的表现，达到开源模型的最新水平。

Conclusion: TAPS为个性化工具使用提供了有效解决方案，推动了目标导向对话代理的发展。
...
<details>
  <summary>Details</summary>
Motivation: 罕见病诊断面临临床异质性、低流行率和医生知识有限的挑战，需要高效准确的诊断工具。

Method: DeepRare由中央主机、长期记忆模块和多个专业代理服务器组成，整合40多种工具和最新医学知识。

Result: 在2919种疾病中达到100%准确率，HPO评估中显著优于其他方法，多模态输入场景下表现优异。

Conclusion: DeepRare展示了罕见病诊断的高效性和透明性，已作为用户友好型网络应用实现。
...
Method: CoDoT将自然语言输入转换为代表相同意图的简单代码，以此评估模型的安全性。

Result: 实验表明，CoDoT导致多种先进LLMs在安全性上表现不佳，例如GPT-4 Turbo的毒性增加了16.5倍。

Conclusion: CoDoT强调了从基本原理评估安全性的重要性，以确保安全性和能力同步提升。
...
<details>
  <summary>Details</summary>
Motivation: 研究对话中说话时间分配的动态过程及其对参与者感知的影响，为计算机辅助沟通平台设计提供工具。

Method: 通过计算框架量化对话级说话时间分配及低层动态，并应用于大型视频聊天数据集。

Result: 平衡对话更受欢迎，不同动态类型即使导致相同平衡水平也会影响参与者感知。

Conclusion: 框架为设计人机或人人沟通平台提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型部署和推理时面临模型尺寸过大的挑战，现有剪枝方法多针对单一模型。

Method: 将模型压缩问题转化为零阶优化问题，支持层移除、选择和合并三种操作。

Result: 在Llama2-13B模型上，压缩后模型性能保留97.3%，参数减少25%，优于现有方法。

Conclusion: 该方法为高效压缩大语言模型提供了新思路，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在适应外部库API频繁更新时表现不佳，需要一种方法提升其在动态环境中的可靠性。

Method: 构建数据集训练模型进行版本迁移，引入改进的字符串相似度指标作为强化学习的奖励。

Result: ReCode显著提升模型在动态API场景中的表现，尤其在未见的CodeUpdateArena任务上，且对通用能力影响较小。

Conclusion: ReCode是一种有效的方法，适用于多种大语言模型和强化学习算法，显著提升动态API环境中的代码生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 研究基础语言模型在强化学习中的适用性，以推动下一代RL可扩展的基础模型发展。

Method: 通过分析Qwen和Llama模型家族，研究中训练策略对RL动态的影响，并提出两阶段训练策略Stable-then-Decay。

Result: 发现高质量数学语料和长链推理数据对RL性能有显著提升，但需注意数据格式化和训练稳定性。

Conclusion: 提出的Stable-then-Decay策略显著提升了模型在RL中的表现，缩小了与RL友好模型家族的差距，并开源了相关资源。
...
<details>
  <summary>Details</summary>
Motivation: 当前研究集中在英语和少数领域（如数学和代码），缺乏对多语言和开放任务的泛化能力。本文旨在填补这一空白。

Method: 提出新的采样和选择策略，适应多语言和多任务场景，并在8B和111B模型上进行评估。

Result: 新策略在m-ArenaHard-v2.0基准测试中显著提升胜率（8B模型+6.8，111B模型+9.0）。

Conclusion: 语言和任务感知的推理计算方法对性能提升至关重要，尤其在非主流语言中。
...
<details>
  <summary>Details</summary>
Motivation: 部署基于LLM的代理在高风险领域可能引发安全和伦理风险，如物理伤害或财务损失，因此需要一种高效的方法来引导其行为。

Method: 将代理行为引导任务建模为模型编辑任务，提出Behavior Editing方法，并引入多层级基准BehaviorBench进行系统评估。

Result: Behavior Editing能动态调整代理行为，实现局部场景调整和全局道德对齐，既能促进道德行为，也能诱导有害行为。
...
<details>
  <summary>Details</summary>
Motivation: 探索dLLMs在代码生成中的优势，弥补当前训练和推理机制的不足，揭示其与自回归模型的不同解码行为。

Method: 训练7B参数的dLLM模型DiffuCoder，分析其解码行为，并提出coupled-GRPO强化学习采样方案。

Result: DiffuCoder在代码生成基准上性能提升4.4%，减少了对自回归因果性的依赖。

Conclusion: dLLMs在代码生成中具有独特优势，coupled-GRPO方法有效提升了性能，为dLLM的生成机制提供了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型在推理与检索紧密耦合任务（如多跳问答）中的局限性。

Method: 将复杂问题分解为小步骤，动态构建事实数据库，整合事实以解决问题。

Result: 在多个基准测试中显著提升性能，如PhantomWiki、2WikiMultiHopQA和MuSiQue。

Conclusion: Memento策略在多跳问答和代理任务中表现出色，为LLMs提供了更高效的推理与检索结合方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前工具难以解释LLMs中动态、多方面的价值观权衡，而认知科学中的认知模型可为此提供框架。

Method: 采用认知模型评估LLMs在两种设置中的价值观权衡：前沿黑盒模型的推理“努力”程度，以及开源模型的RL后训练动态。

Result: 推理模型更注重信息效用，开源模型在数学推理中表现更强；训练早期选择对价值观有持久影响。

Conclusion: 该方法对快速发展的LLM领域具有广泛适用性，可用于形成其他高级行为假设、优化推理模型训练及控制价值观权衡。
...
<details>
  <summary>Details</summary>
Motivation: 农业喷雾器喷杆的不稳定性是喷雾应用误差的主要因素之一，但目前缺乏定量数据来改进喷杆设计和控制系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽视地面真实密度图的极端稀疏性，导致模型在稀疏区域表现不佳，且传统损失函数假设高斯分布，不适合离散计数数据。

Method: 提出EBC-ZIP框架，采用零膨胀泊松回归的负对数似然损失，结合增强块分类框架，改进稀疏区域性能。

Result: 在四个基准测试中，EBC-ZIP优于现有方法，达到最先进性能。

Conclusion: EBC-ZIP通过更合理的概率损失函数，有效解决了人群计数中的稀疏性和分布不平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有令牌合并方法主要依赖特征相似性，忽略了早期层中空间信息的重要性。

Method: 提出ToSA，利用深度图像生成伪空间令牌，结合语义和空间信息指导令牌合并。

Result: 在多个基准测试中优于现有方法，显著减少ViT运行时间。

Conclusion: ToSA是一种高效的ViT加速解决方案，代码已开源。
...
<details>
  <summary>Details</summary>
Motivation: AI生成视频的视觉伪影（如运动不一致、物理不合理的轨迹等）影响真实性和用户信任，但缺乏针对伪影定位的综合基准。

Method: 引入BrokenVideos数据集，包含3,254个AI生成视频，带有像素级标注的伪影区域，并通过人工验证确保标注质量。

Result: 实验表明，基于BrokenVideos训练的最先进模型在伪影定位能力上有显著提升。

Conclusion: BrokenVideos为生成视频模型的伪影定位研究提供了关键基准，推动了该领域的进展。
...
Method: 通过引入概念框架，系统梳理从2D到3D的世界模型技术，重点分析3D表示和世界知识两大驱动力，以及3D物理场景生成、空间推理和交互能力。

Result: 论文总结了3D世界模型在具身AI、自动驾驶、数字孪生和游戏/VR等领域的应用，并指出了数据、建模和部署中的挑战。

Conclusion: 未来需要进一步推动更鲁棒和通用的3D世界模型发展，解决现有挑战。
...
Method: 提出Windowed Gradient Accumulation (WGA)策略和Thresholded Loss Masking (TLM)策略，并开发ECGVF基准用于评估。

Result: 实验表明EAR在擦除效果和模型效用保留方面均有显著提升。

Conclusion: EAR方法有效解决了自回归模型中的概念擦除问题，并提供了新的评估基准。
...
<details>
  <summary>Details</summary>
Motivation: 传统剪枝方法需要分阶段进行训练、剪枝和微调，效率较低。本文旨在通过一种剪枝与训练结合的方法，自动选择剪枝标准和层，以提升剪枝效率和模型性能。

Method: 提出LAASP方法，采用剪枝与训练结合的单周期流程，自动选择剪枝标准和层，并通过网络损失指导选择。每减少一定FLOPs后进行短暂重训练以缓解精度下降。

Result: 在CIFAR-10和ImageNet数据集上，ResNet56和ResNet110模型的FLOPs减少52%，精度提升；ResNet50模型的FLOPs减少42%，top-5精度仅下降0.33%。

Conclusion: LAASP方法通过自动化和高效的剪枝策略，显著减少了计算量并保持了模型精度，适用于资源受限的边缘设备部署。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型在视觉退化场景下易产生幻觉内容，缺乏对不确定性的感知能力，需改进。

Method: 提出KIE-HVQA基准和GRPO框架，结合视觉不确定性自感知和拒绝回答机制。

Result: 7B参数模型在KIE-HVQA上幻觉减少22%，标准任务性能未显著下降。
...
<details>
  <summary>Details</summary>
Motivation: 探索预训练模型在地球观测任务中的复用与组合潜力，以提升性能并降低资源消耗。

Method: 使用GEO-Bench基准测试评估多个预训练模型（如Prithvi、Hiera和DOFA），通过特征级集成和知识蒸馏技术。

Result: 小型预训练模型的特征级集成性能优于或接近大型模型，且资源需求更低。

Conclusion: 预训练模型的特征级集成和知识蒸馏为地球观测任务提供了一种高效且实用的解决方案。
...
Method: 提出了渐进对齐退化模块（PADM），通过两个子网络（PAlignNet和PDegradeNet）的相互迭代自适应学习准确的退化过程；并引入HFreqdiff，将高频细节嵌入扩散框架，结合CFB和BACM模块进行频率选择性细节提取和精确的反向过程学习。

Result: 实验和消融研究表明，该方法在性能上优于现有技术。

Conclusion: 提出的方法有效解决了Wald协议的局限性，显著提升了多光谱图像融合的质量和空间清晰度。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于码本的方法要么词汇量小（约16K条目），缺乏细粒度语义，要么简单扩展导致标记利用率低和训练不稳定。

Method: 通过聚类数百万SigLIP序列嵌入，构建了一个50万条目的码本，采用级联设计（冻结码本锚定嵌入空间，可训练码本优化任务特定语义）确保稳定性。

Result: UniCode^2在多种基准测试中表现优异，支持高质量视觉合成，且无需牺牲稳定性、语义或模块性。
...
<details>
  <summary>Details</summary>
Motivation: 手术流程的复杂性和多样性导致通用模型难以适应不同机构和手术环境，现有基础模型的零样本性能受限于领域偏移。

Method: SPA结合少量样本空间适应、扩散建模确保时间一致性，以及动态测试时自适应，提升模型在未见手术环境中的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 手写识别通常仅利用单一模态（如字形或笔画轨迹），而忽略了二者的互补性。

Method: 设计了一个端到端网络，将离线图像和在线笔画数据在共享潜在空间中进行早期融合，使用补丁编码器和轻量级Transformer分别处理图像和笔画序列。

Result: 在IAMOn-DB和VNOn-DB数据集上实现了最优性能，准确率提升达1%。
...
Conclusion: 该方法为艺术品分析开辟了新途径，尤其适用于线密度相似的平纹画布。
...
TL;DR: 论文提出了DenseWorld基准和DenseDiT方法，用于解决密集预测任务在真实场景中的泛化问题，并通过轻量级设计显著减少参数需求。
...
Method: 提出DenseDiT，利用生成模型的视觉先验，通过参数重用机制和两个轻量级分支实现多尺度上下文自适应整合。

Result: 在DenseWorld基准上，DenseDiT性能优于现有基线，且仅需不到0.01%的训练数据。
...
TL;DR: 该论文提出了一种从光谱域解决未注册高光谱图像（HSI）和多光谱图像（MSI）融合问题的方法，通过轻量级光谱先验学习（SPL）网络和盲稀疏融合（BSF）方法，提高了注册和融合效果，并降低了计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过空间变换对齐HSI和MSI，但由于空间分辨率差异大，效果不佳且耗时。论文旨在从光谱域解决注册问题，提高融合效率和准确性。

Method: 1. 使用SPL网络提取HSI的光谱特征并增强MSI的光谱分辨率；2. 通过子空间表示和循环训练策略提高注册HSI的光谱精度；3. 提出BSF方法，利用群稀疏正则化等效促进图像的低秩性；4. 采用PAO算法求解BSF模型。

Result: 实验验证了该方法在注册和融合中的有效性，并展示了其在分类性能提升方面的优势。

Conclusion: 论文提出的方法从光谱域解决了HSI和MSI的注册问题，显著提高了融合效果和计算效率，适用于遥感图像处理。
...
TL;DR: 提出了一种名为Ctrl-Z Sampling的新采样策略，用于在条件生成中检测和逃离局部最优解，从而提高生成质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在条件生成中表现出色，但容易陷入局部最优解，导致全局不一致或条件不对齐。现有方法通过增强引导信号或调整初始噪声分布来缓解这一问题，但效果有限。

Method: Ctrl-Z Sampling通过奖励模型识别局部最优解，注入噪声并回退到更早的状态以逃离优化平台。奖励模型评估候选轨迹，仅接受改进的方案，逐步深入的回退机制增强了逃离能力。

Result: 实验表明，Ctrl-Z Sampling显著提升了生成质量，仅增加约7.6倍的函数评估次数。

Conclusion: Ctrl-Z Sampling是一种模型无关的方法，兼容现有扩散框架，能有效提升条件生成的对齐性和视觉质量。
...
Conclusion: 扩散模型与Transformer的结合能有效提升退化图像的质量，扩展其在需要高保真视觉数据的下游任务中的应用。
...
TL;DR: 提出了一种动态构建放射组学特征（指纹）的新方法，通过深度学习模型为每位患者个性化选择特征，提高了诊断准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统放射组学方法使用固定特征集，无法捕捉个体病理差异，导致性能受限。本文旨在解决这一问题。

Method: 提出放射组学指纹框架，动态为每位患者选择特征，结合可解释的逻辑回归进行分类。

Result: 在多项诊断任务中表现优于或媲美端到端深度学习模型，同时保持了可解释性。

Conclusion: 该方法不仅提高了诊断准确性，还通过可解释性为临床提供了有价值的见解。
...
TL;DR: 本文评估了五种目标检测架构在三个历史文档数据集上的表现，发现CNN-OBB模型在复杂文档中表现最佳，而Transformer模型在结构化布局中更优。


<details>
  <summary>Details</summary>
Motivation: 历史文档布局复杂，需要鲁棒的文档布局分析（DLA）技术来自动化处理。

Method: 使用五种目标检测架构（包括两种Transformer和三种YOLO变体）在三个数据集（e-NDP、CATMuS、HORAE）上进行测试。

Result: Co-DETR在e-NDP数据集上表现最佳（0.752 mAP），而YOLOv11x-OBB在更复杂的CATMuS和HORAE数据集上显著优于其他模型（0.564和0.568 mAP）。

Conclusion: OBB是建模历史文档非笛卡尔特性的关键，Transformer适合结构化布局，而CNN-OBB模型在复杂文档中泛化能力更强。
...
TL;DR: 提出了一种深度翻译动作识别框架，通过联合预测动作概念和辅助特征提升识别精度，并引入两种新颖的域特定描述符（ODF和SDF）来增强特征表示。


<details>
  <summary>Details</summary>
Motivation: 视频中的人类动作识别需要高层次的语义推理和多模态特征的整合，而现有方法在特征表示和计算效率上存在不足。

Method: 提出了一种框架，结合对象检测特征（ODF）和显著性检测特征（SDF），并整合多种辅助模态（如光流、骨架数据等），同时引入不确定性建模和鲁棒损失函数。

Result: 在多个基准测试（如Kinetics-400、Kinetics-600等）上实现了最先进的性能。

Conclusion: 该框架能有效捕捉细粒度动作动态，且兼容多种先进架构，具有广泛的应用潜力。
...
Method: 采用噪声对抗学习训练特征提取器，结合对抗监督和重构约束；设计基于学习的多比特零水印方案，将不变特征投影到可训练参考码上。

Result: 在多种图像数据集和失真条件下表现出优异的鲁棒性，特征稳定性和水印恢复能力达到最优。
...
TL;DR: HiT和DyHiT是高效的视觉跟踪模型，通过Bridge Module和动态路由技术，在保持高性能的同时提升速度。DyHiT还能动态适应场景复杂度，进一步优化速度与精度平衡。
...
Method: HiT采用Bridge Module连接轻量级Transformer；DyHiT通过动态路由分类场景并选择计算路径。

Result: HiT在NVIDIA Jetson AGX上达到61 fps，AUC 64.6%；DyHiT最快111 fps，AUC 62.4%。

Conclusion: HiT和DyHiT在速度和精度上取得平衡，动态路由方法还能加速其他高性能跟踪器。
...
TL;DR: Med-Art是一个针对有限数据医学图像生成的框架，利用视觉语言模型和预训练模型PixArt-α，通过HLDF方法解决数据稀缺和颜色过饱和问题，取得了最优性能。
...
Method: 结合视觉语言模型生成医学图像描述，采用PixArt-α模型和HLDF方法进行微调。

Result: 在两个医学图像数据集上实现了FID、KID和分类性能的最优结果。
...
TL;DR: HiWave是一种无需训练的方法，通过两阶段流程（基础图像生成和基于小波的细节增强模块）提升超高分辨率图像合成的视觉保真度和结构一致性。


<details>
  <summary>Details</summary>
Motivation: 解决现有零射生成技术在超高分辨率图像合成中产生的伪影（如对象重复和空间不连贯）问题。

Method: 采用两阶段流程：1）生成基础图像；2）通过DDIM反演和小波域细节增强模块优化细节。

Result: HiWave显著减少了伪影，在用户研究中80%的情况下优于现有技术。

Conclusion: HiWave是一种高效且无需重新训练的超高分辨率图像合成方法。
...
TL;DR: 论文提出了一种名为DeepBolt的两阶段深度学习架构，用于在复杂3D点云中自动高效识别岩石螺栓，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 地下矿井中岩石螺栓的频繁评估对维持岩体稳定性至关重要，但手动检测困难，现有自动化方法因数据噪声和环境复杂性缺乏鲁棒性。

Method: 采用新颖的两阶段深度学习架构DeepBolt，专门处理类别不平衡问题，用于3D点云中的岩石螺栓识别。

Result: DeepBolt在岩石螺栓点的IoU上比现有语义分割模型高出42.5%，分类精度和召回率分别达到96.41%和96.96%。

Conclusion: DeepBolt在复杂地下环境中表现出高效性和鲁棒性，为岩石螺栓的自动化检测提供了可靠解决方案。
...
TL;DR: 提出一种基于AI的深度学习框架，用于自动检测和量化牙槽骨流失及其模式，提高牙周病诊断和治疗规划的准确性。
...
Method: 结合YOLOv8进行牙齿检测，使用Keypoint R-CNN模型识别解剖标志点，并通过YOLOv8x-seg模型分割骨水平和牙齿掩模，进行几何分析以确定骨流失模式。

Result: 在1000张放射线照片的数据集上，骨流失严重程度检测的类内相关系数高达0.80，骨流失模式分类准确率为87%。

Conclusion: 该自动化系统为牙周评估提供了快速、客观且可重复的工具，有望改善早期诊断和个性化治疗规划，提升患者护理和临床效果。
...
TL;DR: PLADA框架通过处理压缩图像的块效应和利用配对与非配对数据，显著提升了社交媒体上深度伪造图像的检测性能。


<details>
  <summary>Details</summary>
Motivation: 深度伪造图像在社交媒体上广泛传播，现有检测方法忽视压缩引入的块效应，且依赖原始图像，难以应对实际场景。

Method: PLADA包含块效应消除模块（B2E）和开放数据聚合模块（ODA），分别处理块效应和利用多种数据提升检测。

Result: 在26个数据集上的实验表明，PLADA在深度伪造检测中表现优异，尤其在压缩和有限配对数据条件下。
...
TL;DR: 论文提出了一种简单有效的策略，通过将连续多帧堆叠输入YOLO检测器，仅监督目标帧输出，以利用时间信息提升检测性能。
...
Conclusion: 该方法简单高效，适用于实时检测，并贡献了新数据集BOAT360。
...
Method: 提出对抗性掩码图像建模方法，通过掩码输入预测完整分割掩码，利用原始标签和伪标签增强监督信号，并设计对抗训练损失减少域差距。
...
TL;DR: 提出了一种基于分割与摘要（DaS）的密集视频字幕框架，通过将长视频分割为多个事件提案，并利用视觉特征和LSTM网络生成描述性句子。


<details>
  <summary>Details</summary>
Motivation: 解决长视频中密集事件描述的挑战，通过分割和摘要方法提高字幕生成的准确性和效率。

Method: 将视频分割为事件提案，提取视觉特征并生成句子描述，然后使用两阶段LSTM网络（编码器和解码器）结合分层注意力机制进行句子摘要。

Result: 在ActivityNet Captions数据集上的实验验证了DaS框架的有效性。

Conclusion: DaS框架通过分割与摘要方法显著提升了密集视频字幕生成的性能。
...
Method: 采用端到端框架，通过分组观察学习可识别因果表示，强制模型对种族、性别和成像视图等变量保持不变性。

Result: 实验表明，这种因果表示在多个分类任务中显著提高了泛化性和鲁棒性。
...
TL;DR: 论文提出了一种基于图的分割与总结（GPaS）框架，用于密集视频描述，通过分割事件提案为更短的片段并总结生成的句子，解决了现有方法在场景变化时表现不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 现有密集视频描述方法未能充分探索事件提案内的场景演变，导致在场景和对象变化较大的情况下表现不佳。

Method: 提出GPaS框架，分为分割和总结两阶段：分割阶段将事件提案拆分为更短的片段进行描述；总结阶段通过图卷积网络（GCN）和长短时记忆网络（LSTM）结合，学习语义词之间的关系以生成总结句子。

Result: 在ActivityNet Captions和YouCook II数据集上，该方法优于现有技术。

Conclusion: GPaS框架通过细粒度分割和语义词关系建模，显著提升了密集视频描述的性能。
...
TL;DR: 提出一种基于神经网络的单目360度鱼眼相机距离估计方法，优于传统几何方法和其他学习基线。


<details>
  <summary>Details</summary>
Motivation: 解决传统几何方法在全向成像中因镜头畸变和环境变化导致的距离估计不准确问题。
...
<details>
  <summary>Details</summary>
Motivation: 视频内容日益普及，需要高效访问关键信息，但现有方法依赖监督标注或计算昂贵的注意力模型，限制了跨域适用性。

Method: 采用自监督学习框架，结合马尔可夫过程驱动的损失指标和两阶段训练，避免使用注意力、RNN或Transformer。

Result: 在SUMME和TVSUM数据集上达到最先进性能，优于所有无监督方法，并与最佳监督模型媲美。

Conclusion: 展示了高效、无需标注的架构潜力，挑战了对复杂架构的依赖，推动了更通用的视频摘要技术。
...
TL;DR: WonderFree是一种交互式3D场景生成模型，解决了现有方法在探索性和视角一致性上的不足，通过WorldRestorer和ConsistView技术提升渲染质量和全局一致性。
...
Method: 提出WonderFree模型，包括WorldRestorer（消除新视角中的视觉伪影）和ConsistView（多视角联合恢复机制）。

Result: 实验表明，WonderFree显著提升了渲染质量和全局一致性，用户偏好率达77.20%。

Conclusion: WonderFree为3D场景生成提供了更高质量的探索体验，代码和模型将公开。
...
TL;DR: SFNet提出了一种新的遥感图像伪造检测框架，通过结合空间和频域特征，显著提高了检测准确性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的快速发展导致伪造遥感图像难以检测，现有方法依赖单一视觉特征，难以应对多样化的伪造痕迹。

Method: SFNet采用两个独立的特征提取器捕获空间和频域特征，并通过域特征映射和混合域特征细化模块（CBAM注意力）对齐和融合多域特征。

Result: 在三个数据集上的实验显示，SFNet比现有方法准确率提高了4%-15.18%，并表现出强大的泛化能力。
...
TL;DR: VIPScene利用视频生成模型的3D物理世界知识，实现高真实性和结构一致性的3D场景合成。


<details>
  <summary>Details</summary>
Motivation: 传统3D场景合成需要专家知识和大量手动工作，自动化此过程对建筑设计、机器人模拟等领域有重要意义。现有方法（如LLMs或图像生成模型）在3D空间推理或多视角一致性上存在局限。

Method: VIPScene结合视频生成、3D重建和开放词汇感知模型，通过文本和图像提示生成场景，并引入FPVScore评估一致性和合理性。

Result: 实验表明VIPScene显著优于现有方法，且能泛化到多样场景。

Conclusion: VIPScene提供了一种灵活且高真实性的3D场景合成框架，代码将开源。
...
Method: 使用开放词汇分割提取物体轮廓，利用视觉语言模型解释动物概念，并通过文本到图像扩散模型合成动物图像。

Result: 在多样化真实输入上验证了框架的鲁棒性和创造力。

Conclusion: Shape2Animal为视觉叙事、教育内容和交互媒体设计提供了新工具。
...
TL;DR: MMSearch-R1是一个端到端强化学习框架，用于优化大型多模态模型（LMMs）在真实互联网环境中的多轮搜索行为，通过整合图像和文本搜索工具，减少搜索调用次数并提升性能。
...
Method: 提出MMSearch-R1框架，结合图像和文本搜索工具，通过基于结果的奖励和搜索惩罚机制，训练模型动态决定何时及如何调用搜索工具。

Result: 实验表明，MMSearch-R1在知识密集和信息寻求任务中优于同规模RAG基线，并减少30%以上的搜索调用，性能与更大RAG模型相当。

Conclusion: MMSearch-R1为多模态搜索研究提供了高效、按需的解决方案，并通过数据集和分析为未来研究提供参考。
...
Method: 采用解耦表示学习（DRL）框架，通过从合成数据中学习表示并迁移到真实数据，提升可解释性。

Result: 在三个显微镜图像数据集（浮游生物、酵母液泡和人类细胞）上验证了该方法在准确性和可解释性之间的良好平衡。
...
Conclusion: IPFormer通过动态实例提案和注意力机制，为基于视觉的3D全景场景完成提供了创新解决方案。
...
### [82] [Accurate and Energy Efficient: Local Retrieval-Augmented Generation Models Outperform Commercial Large Language Models in Medical Tasks](https://arxiv.org/abs/2506.20009)
*Konstantinos Vrettos,Michail E. Klontzas*

Main category: cs.AI

TL;DR: 研究发现，基于本地LLM的RAG框架在医疗任务中优于商业模型，同时减少能源消耗和碳排放。
...
Method: 开发可定制的RAG框架，监测能源使用和CO2排放，并基于开源LLM构建模型。

Result: 本地RAG模型在准确性和能源效率上优于商业模型，llama3.1:8B表现最佳。

Conclusion: 本地LLM的RAG框架在医疗任务中更高效且环保，支持可持续发展目标。
...
### [83] [Persona-Assigned Large Language Models Exhibit Human-Like Motivated Reasoning](https://arxiv.org/abs/2506.20020)
*Saloni Dash,Amélie Reymond,Emma S. Spiro,Aylin Caliskan*

Main category: cs.AI

TL;DR: 研究发现，大型语言模型（LLM）在赋予政治和社会身份后，会表现出类似人类的动机性推理，导致判断偏差，且传统去偏方法效果有限。


<details>
  <summary>Details</summary>
Motivation: 探讨LLM是否会在身份认同驱动下进行动机性推理，及其对社会议题（如气候变化、疫苗安全）的影响。

Method: 通过为8种LLM赋予4类政治和社会身份，测试其在信息真实性辨别和科学证据评估任务中的表现。

Result: 身份赋予使LLM的真实性辨别能力下降9%，政治身份模型在符合其身份的议题上表现更好，但去偏提示效果有限。

Conclusion: LLM表现出类似人类的动机性推理，可能加剧身份认同驱动的判断偏差，需进一步研究应对方法。
...
### [84] [Language Modeling by Language Models](https://arxiv.org/abs/2506.20249)
*Junyan Cheng,Peter Clark,Kyle Richardson*

Main category: cs.AI

TL;DR: 利用多智能体LLM方法Genesys，通过模拟研究流程（从构思到验证）自动发现新型语言模型架构，其遗传编程方法显著优于直接提示生成，并在实验中验证了1,062个设计，部分性能超越现有架构。


<details>
  <summary>Details</summary>
Motivation: 探索是否可以利用LLM模拟研究流程，自动发现高效的语言模型架构，以解决手动设计的高成本和低效率问题。

Method: 提出Genesys系统，采用多智能体LLM模拟研究流程（提案、代码生成、预训练、验证），结合遗传编程和Ladder of Scales方法逐步验证设计。

Result: 实验验证了1,062个新设计，部分性能超越GPT2和Mamba2等现有架构，遗传编程方法在生成成功率上提升86%。

Conclusion: Genesys证明了LLM在自动发现高效模型架构上的潜力，为自主研究系统设计提供了新思路。
...
### [85] [The Decrypto Benchmark for Multi-Agent Reasoning and Theory of Mind](https://arxiv.org/abs/2506.20664)
*Andrei Lupu,Timon Willi,Jakob Foerster*

Main category: cs.AI

TL;DR: Decrypto是一个基于游戏的基准测试，用于评估大语言模型在多智能体推理和心理理论（ToM）方面的能力，填补了现有基准的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准在多智能体推理和ToM评估中存在范围狭窄、数据泄漏、饱和度和缺乏交互性等问题，需要新的评估工具。

Method: 提出Decrypto基准，结合认知科学、计算语用学和多智能体强化学习，设计交互式实验平台。

Result: 前沿大语言模型在游戏能力上落后于人类和简单词嵌入基线，且最新推理模型在ToM任务中表现不如旧模型。

Conclusion: Decrypto填补了当前推理和ToM评估的空白，为开发更优的人工智能代理铺平道路。
...
### [86] [Practical insights on the effect of different encodings, ansätze and measurements in quantum and hybrid convolutional neural networks](https://arxiv.org/abs/2506.20355)
...
TL;DR: 研究了参数化量子电路（PQCs）在量子卷积神经网络（QCNN）和混合量子卷积神经网络（HQNN）中的设计选择，用于卫星图像分类任务。分析了数据编码、变分结构和测量对性能的影响。
...
Result: 混合架构中，数据编码策略对性能影响最大（验证准确率差异达30%），而变分结构和测量方法影响较小（差异低于5%）。纯量子模型中，测量协议和数据映射对性能影响显著（验证准确率差异达30%和8%）。

Conclusion: 数据编码策略是混合量子架构性能的关键因素，而纯量子模型更依赖于测量协议和数据映射。
...
### [87] [FundaQ-8: A Clinically-Inspired Scoring Framework for Automated Fundus Image Quality Assessment](https://arxiv.org/abs/2506.20303)
...
<details>
  <summary>Details</summary>
Motivation: 眼底图像质量评估（FIQA）因图像采集差异和主观专家评估而具有挑战性，需要一种系统化的方法。

Method: 开发FundaQ-8框架，基于八个参数评估图像质量；使用ResNet18回归模型预测质量分数，采用迁移学习和标准化预处理。

Result: 模型在EyeQ数据集上验证可靠，且能提升糖尿病视网膜病变分级的诊断稳健性。

Conclusion: FundaQ-8结合深度学习模型可提高眼底图像质量评估的可靠性和临床实用性。
...
### [88] [VoxelOpt: Voxel-Adaptive Message Passing for Discrete Optimization in Deformable Abdominal CT Registration](https://arxiv.org/abs/2506.19975)
*Hang Zhang,Yuxi Zhang,Jiazheng Wang,Xiang Chen,Renjiu Hu,Xin Tian,Gaolei Li,Min Liu*

Main category: eess.IV

TL;DR: VoxelOpt 是一种基于离散优化的图像配准框架，结合了学习和迭代方法的优势，在精度和速度之间取得平衡。


<details>
  <summary>Details</summary>
Motivation: 学习型方法在数据有限或大变形时表现不佳，而迭代方法虽准确但速度慢。VoxelOpt 旨在解决这些问题。

Method: 利用位移熵测量信号强度，引入体素自适应消息传递、多级图像金字塔和预训练分割模型提取特征。

Result: 在腹部 CT 配准中，VoxelOpt 在效率和精度上优于迭代方法，与监督学习型方法相当。

Conclusion: VoxelOpt 提供了一种高效且准确的图像配准解决方案。
...
### [89] [MS-IQA: A Multi-Scale Feature Fusion Network for PET/CT Image Quality Assessment](https://arxiv.org/abs/2506.20200)
*Siqiao Li,Chen Hui,Wei Zhang,Rui Liang,Chenyue Song,Feng Jiang,Haiqi Zhu,Zhixuan Li,Hong Huang,Xiang Li*

Main category: eess.IV

TL;DR: 提出了一种多尺度特征融合网络（MS-IQA）用于PET/CT图像质量评估，结合高低层次特征，并构建了包含2700张图像的数据集。


<details>
  <summary>Details</summary>
Motivation: 现有医学图像质量评估方法无法同时考虑低层次和高层次特征，可能导致诊断不确定性。

Method: 利用ResNet和Swin Transformer的多尺度特征，引入动态加权通道注意力机制融合特征。

Result: 在PET-CT-IQA-DS和LDCTIQAC2023数据集上表现优于现有方法。

Conclusion: MS-IQA为PET/CT提供了准确高效的图像质量评估方法。
...
### [90] [Opportunistic Osteoporosis Diagnosis via Texture-Preserving Self-Supervision, Mixture of Experts and Multi-Task Integration](https://arxiv.org/abs/2506.20282)
*Jiaxing Huang,Heng Guo,Le Lu,Fan Yang,Minfeng Xu,Ge Yang,Wei Luo*

Main category: eess.IV

TL;DR: 提出了一种深度学习框架，通过自监督学习、混合专家架构和多任务学习，解决了骨质疏松诊断中的未标记数据利用、设备差异和临床知识整合问题。


<details>
  <summary>Details</summary>
Motivation: 骨质疏松症诊断在资源有限地区受限，现有CT分析方法存在未标记数据利用不足、设备差异和临床知识整合不充分的问题。

Method: 采用自监督学习利用未标记CT数据，混合专家架构增强跨设备适应性，多任务学习整合骨质疏松诊断、BMD回归和椎体定位。

Result: 在三个临床站点和外部医院验证中，该方法表现出优于现有方法的泛化能力和准确性。

Conclusion: 该框架为骨质疏松症的筛查和诊断提供了一种高效、通用的解决方案。
...
### [91] [EAGLE: An Efficient Global Attention Lesion Segmentation Model for Hepatic Echinococcosis](https://arxiv.org/abs/2506.20333)
...
Result: 在260例患者的CT数据上，EAGLE的DSC达到89.76%，优于MSVM-UNet 1.61%。
...
### [92] [Fusing Radiomic Features with Deep Representations for Gestational Age Estimation in Fetal Ultrasound Images](https://arxiv.org/abs/2506.20407)
*Fangyijie Wang,Yuan Liang,Sourav Bhattacharjee,Abey Campbell,Kathleen M. Curran,Guénolé Silvestre*

Main category: eess.IV

TL;DR: 提出了一种基于特征融合的框架，通过胎儿超声图像自动估计孕龄，无需测量信息，性能优于现有方法。
...
### [93] [Weighted Mean Frequencies: a handcraft Fourier feature for 4D Flow MRI segmentation](https://arxiv.org/abs/2506.20614)
*Simon Perrin,Sébastien Levilly,Huajun Sun,Harold Mouchère,Jean-Michel Serfaty*

Main category: eess.IV

TL;DR: 提出了一种名为加权平均频率（WMF）的新特征，用于改善4D Flow MRI图像的分割任务，显著提升了分割效果。


<details>
  <summary>Details</summary>
Motivation: 4D Flow MRI图像的分辨率低和噪声问题影响了生物标志物的准确性，尤其是壁剪切应力。PC-MRA是目前的分割方法，但仍有改进空间。

Method: 提出WMF特征，用于可视化4D Flow MRI图像中的脉动流区域，并通过阈值优化和深度学习方法进行分割实验。

Result: 实验结果显示，WMF特征在IoU和Dice指标上分别比PC-MRA特征提高了0.12和0.13。

Conclusion: WMF特征在分割任务中表现优异，未来可应用于其他血管区域的分割。
...
### [94] [Capturing Visualization Design Rationale](https://arxiv.org/abs/2506.16571)
*Maeve Hutchinson,Radu Jianu,Aidan Slingsby,Jo Wood,Pranava Madhyastha*

Main category: cs.HC

TL;DR: 该论文提出了一种新的数据集和方法，通过自然语言探究可视化设计原理，利用学生创建的可视化笔记本和大型语言模型（LLMs）生成问题-答案-原理三元组。


<details>
  <summary>Details</summary>
Motivation: 现有自然语言数据集多关注可视化解读而非设计原理，且多为人工构建的受控实验。本文旨在填补这一空白，通过真实世界的可视化笔记本理解设计决策背后的原理。

Method: 利用学生数据可视化课程中的笔记本（含可视化作品和设计说明），结合LLMs生成问题-答案-原理三元组，并验证和整理数据集。
...
### [95] [A Multi-Modal Spatial Risk Framework for EV Charging Infrastructure Using Remote Sensing](https://arxiv.org/abs/2506.19860)
...
<details>
  <summary>Details</summary>
Motivation: 电动汽车充电基础设施的韧性在环境和基础设施压力下尚未充分研究，需要一种综合评估方法。

Method: 结合遥感数据、基础设施数据集和空间图分析，构建多模态风险评估框架RSERI-EV，生成综合韧性评分。
...
### [96] [A Spatio-Temporal Point Process for Fine-Grained Modeling of Reading Behavior](https://arxiv.org/abs/2506.19999)
*Francesco Ignazio Re,Andreas Opedal,Glib Manaiev,Mario Giulianelli,Ryan Cotterell*

Main category: cs.LG

TL;DR: 论文提出了一种基于时空点过程的阅读行为概率模型，捕捉注视的持续时间、空间位置和时间点，使用霍克斯过程模拟扫视，模型拟合效果优于基线。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖聚合的眼动追踪数据和强假设模型，忽略了阅读中的时空动态。本文旨在提出更通用的模型以更好地理解阅读行为。

Method: 采用标记的时空点过程模型，扫视用霍克斯过程模拟，注视持续时间通过时间卷积的预测变量建模。

Result: 霍克斯过程模型在模拟人类扫视时优于基线；注视持续时间模型中，上下文意外性作为预测变量仅带来边际改进。

Conclusion: 意外性理论难以解释精细的眼动行为，新模型为阅读行为提供了更全面的框架。
...
### [97] [MIRAGE: A Benchmark for Multimodal Information-Seeking and Reasoning in Agricultural Expert-Guided Conversations](https://arxiv.org/abs/2506.20100)
*Vardhan Dongre,Chi Gui,Shubham Garg,Hooshang Nayyeri,Gokhan Tur,Dilek Hakkani-Tür,Vikram S. Adve*

Main category: cs.LG

TL;DR: MIRAGE是一个用于多模态专家级推理和决策的新基准，专注于农业领域，结合自然用户查询、专家回答和图像上下文，支持复杂场景下的评估。


<details>
  <summary>Details</summary>
Motivation: 现有基准通常依赖明确输入和封闭分类，而MIRAGE旨在解决开放世界场景中的未明确问题和罕见实体处理需求。

Method: 基于35,000多个真实用户-专家交互，通过多步流程构建，涵盖7,000多种生物实体，包括植物、害虫和疾病。

Result: MIRAGE成为视觉语言模型中分类最多样化的基准之一，支持真实世界的复杂推理和长文本生成。
...
### [98] [Position: Machine Learning Conferences Should Establish a "Refutations and Critiques" Track](https://arxiv.org/abs/2506.19882)
...
TL;DR: 本文主张机器学习会议应设立专门的“反驳与批评”（R&C）轨道，以系统性纠正研究中的错误，促进自我修正的研究生态系统。


<details>
  <summary>Details</summary>
Motivation: 机器学习研究快速发展导致大量论文发表，但同行评审的局限性可能导致错误或误导性研究被接受甚至突出展示，缺乏系统性纠正机制。

Method: 提出在机器学习会议中设立R&C轨道，讨论其设计、评审原则、潜在问题，并以ICLR 2025 Oral为例说明。

Result: 通过R&C轨道为批评性研究提供高声誉平台，促进研究生态系统的自我修正。
...
### [99] [Counterfactual Influence as a Distributional Quantity](https://arxiv.org/abs/2506.20481)
*Matthieu Meeus,Igor Shilov,Georgios Kaissis,Yves-Alexandre de Montjoye*

Main category: cs.LG

TL;DR: 论文研究了机器学习模型记忆训练数据的现象，提出仅依赖自影响（self-influence）会低估记忆风险，而应考虑所有训练样本的完整影响分布。


<details>
  <summary>Details</summary>
Motivation: 机器学习模型记忆训练数据可能引发隐私和泛化问题，现有方法仅关注自影响，忽略了其他样本（如近重复样本）的影响。

Method: 通过计算训练样本间的完整影响分布，分析其对记忆化的作用，并在小语言模型和图像分类任务中验证。

Result: 发现自影响严重低估记忆风险，近重复样本虽自影响低但仍可被提取；CIFAR-10中通过影响分布揭示了近重复样本的存在。

Conclusion: 记忆化是训练数据间复杂相互作用的结果，完整影响分布比单一自影响更能准确捕捉这一现象。
...
### [100] [Asymmetric REINFORCE for off-Policy Reinforcement Learning: Balancing positive and negative rewards](https://arxiv.org/abs/2506.20520)
*Charles Arnal,Gaëtan Narozniak,Vivien Cabannes,Yunhao Tang,Julia Kempe,Remi Munos*

Main category: cs.LG

TL;DR: 论文研究了介于离策略强化学习和监督微调之间的算法，分析了简单的离策略REINFORCE算法，发现调整基线V可以优化性能。理论分析表明，当V低于期望奖励时，算法具有策略改进保证。实验验证了在随机多臂老虎机任务和LLM微调中的有效性。


<details>
  <summary>Details</summary>
Motivation: 探索离策略强化学习在大型语言模型对齐中的性能优化问题，弥补离策略方法在实现简单和数据效率上的优势与性能不足之间的差距。

Method: 分析了一种简单的离策略REINFORCE算法，通过调整基线V（定义为A=r-V）来优化策略，理论证明V低于期望奖励时具有策略改进保证。

Result: 理论分析表明，离策略更新应更关注正奖励信号；实验验证了算法在随机多臂老虎机和LLM微调任务中的有效性。

Conclusion: 调整基线V可以优化离策略强化学习的性能，特别是在LLM对齐任务中，更关注正奖励信号有助于提升效果。
...
### [101] [PLoP: Precise LoRA Placement for Efficient Finetuning of Large Models](https://arxiv.org/abs/2506.20629)
*Soufiane Hayou,Nikhil Ghosh,Bin Yu*

Main category: cs.LG

TL;DR: PLoP是一种轻量级方法，通过自动识别LoRA适配器的最佳放置位置，显著提升了模型微调效率。


<details>
  <summary>Details</summary>
Motivation: 现有LoRA适配器放置策略缺乏统一结论，不同研究提出了不同模块类型（如注意力模块或MLP模块）的放置建议。PLoP旨在通过理论分析和实验验证，提供一种自动化的适配器放置方法。

Method: PLoP通过直观的理论分析，自动识别预训练模型和微调任务中适合放置LoRA适配器的模块类型。

Result: 实验表明，PLoP在监督微调和强化学习任务中始终优于或至少与常用放置策略相当。

Conclusion: PLoP为LoRA适配器放置提供了一种高效且自动化的解决方案，显著提升了模型微调的性能。
...
### [102] [Any-Order GPT as Masked Diffusion Model: Decoupling Formulation and Architecture](https://arxiv.org/abs/2506.19935)
*Shuchen Xue,Tianyu Xie,Tianyang Hu,Zijin Feng,Jiacheng Sun,Kenji Kawaguchi,Zhenguo Li,Zhi-Ming Ma*

Main category: cs.LG

TL;DR: 本文通过将掩码扩散模型（MDM）置于仅解码器框架中，公平比较了自回归（AR）与MDM范式，并探讨了架构（仅解码器与仅编码器）对MDM的影响。研究发现，标准AO-AR目标可能需改进，且仅解码器MDM在生成速度和困惑度上表现优异。


<details>
  <summary>Details</summary>
Motivation: 比较AR与MDM范式时，架构差异导致不公平对比。本文旨在分离范式与架构影响，为未来模型设计提供参考。

Method: 将MDM置于仅解码器框架中，作为任意顺序自回归（AO-AR）与标准AR比较，并分析架构对MDM的影响。

Result: 仅解码器MDM实现显著生成加速（约25倍）和可比困惑度，而标准AO-AR目标可能需优化。

Conclusion: 研究揭示了范式与架构的关键权衡，为模型设计提供新视角。
...
### [103] [FedBKD: Distilled Federated Learning to Embrace Gerneralization and Personalization on Non-IID Data](https://arxiv.org/abs/2506.20245)
*Yushan Zhao,Jinyuan He,Donglai Chen,Weijie Luo,Chong Xie,Ri Zhang,Yonghong Chen,Yan Xu*

Main category: cs.LG

TL;DR: 论文提出了一种名为FedBKD的无数据蒸馏框架，通过生成对抗网络（GAN）合成数据，实现全局和局部模型之间的双向知识蒸馏，以解决联邦学习中的非独立同分布（non-IID）数据问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习（FL）面临非独立同分布数据挑战，现有方法难以同时实现全局模型的泛化性和局部模型的个性化性能。此外，引入公共数据集可能增加数据泄漏风险。

Method: 提出FedBKD框架，利用GAN生成合成数据，局部模型作为判别器且参数冻结。合成数据用于全局和局部模型间的双向知识蒸馏，提升双方性能。

Result: 在4个基准测试中，FedBKD在不同non-IID设置下均达到最优性能。

Conclusion: FedBKD有效解决了FL中的non-IID数据问题，同时提升了全局和局部模型的性能，避免了公共数据集带来的数据泄漏风险。
...
### [104] [Learning Moderately Input-Sensitive Functions: A Case Study in QR Code Decoding](https://arxiv.org/abs/2506.20305)
*Kazuki Yoda,Kazuhiko Kawamoto,Hiroshi Kera*

Main category: cs.LG

TL;DR: 本文研究了基于学习的QR码解码，发现Transformer模型能突破理论纠错限制，通过学习嵌入文本结构成功解码QR码。


<details>
  <summary>Details</summary>
Motivation: 探索中等输入敏感性的学习函数，特别是QR码解码任务，以填补该领域的研究空白。

Method: 使用Transformer模型进行QR码解码实验，分析其在不同语言和随机字符串上的泛化能力。

Result: Transformer模型能突破理论纠错限制，成功解码QR码，并表现出对数据比特的关注而非纠错比特。

Conclusion: Transformer在QR码解码中表现出独特的机制，为中等输入敏感性的学习任务提供了新思路。
...
### [105] [PSALM-V: Automating Symbolic Planning in Interactive Visual Environments with Large Language Models](https://arxiv.org/abs/2506.20097)
*Wang Bill Zhu,Miaosen Chai,Ishika Singh,Robin Jia,Jesse Thomason*

Main category: cs.RO

TL;DR: PSALM-V是一个自主的神经符号学习系统，通过交互在视觉环境中推断符号动作语义，无需专家定义，显著提高了规划成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖文本领域或不切实际的假设（如预定义问题文件或完全可观察性），而PSALM-V旨在动态推断符号语义，适用于部分可观察和多智能体环境。

Method: PSALM-V利用LLM生成启发式计划和候选符号语义，通过分析执行结果动态推断PDDL问题文件和动作语义，迭代优化信念直至达到目标状态。

Result: 在ALFRED任务中，规划成功率从37%提升至74%；在RTFM和Overcooked-AI中提高了步骤效率，并在多智能体环境中成功推断领域语义。

Conclusion: PSALM-V在视觉和多智能体环境中有效推断符号语义，显著提升了规划性能和适应性。
...
### [106] [Why Robots Are Bad at Detecting Their Mistakes: Limitations of Miscommunication Detection in Human-Robot Dialogue](https://arxiv.org/abs/2506.20268)
*Ruben Janssens,Jens De Bock,Sofie Labat,Eva Verhelst,Veronique Hoste,Tony Belpaeme*

Main category: cs.RO

TL;DR: 研究评估机器学习模型在检测人机对话中的沟通错误时的表现，发现即使使用先进模型，识别效果仅略高于随机猜测，揭示了用户反馈不足的根本问题。
...
Conclusion: 研究发现用户即使感知到错误也常未反馈给机器人，这限制了模型的性能，提示需设计更好的反馈机制。
...
### [107] [Consensus-Driven Uncertainty for Robotic Grasping based on RGB Perception](https://arxiv.org/abs/2506.20045)
...
Result: 网络能从多样化的物体中学习，共同训练对目标有帮助。

Conclusion: 多样化的物体数据可以共同提升抓取成功率预测的准确性。
...
### [108] [HRIBench: Benchmarking Vision-Language Models for Real-Time Human Perception in Human-Robot Interaction](https://arxiv.org/abs/2506.20566)
*Zhonghao Shi,Enyu Zhao,Nathaniel Dennler,Jingzhen Wang,Xinyang Xu,Kaleen Shrestha,Mengxue Fu,Daniel Seita,Maja Matarić*

Main category: cs.RO

TL;DR: HRIBench是一个用于评估视觉语言模型（VLMs）在人类感知任务中性能与延迟权衡的基准测试，覆盖五个关键领域。研究发现当前VLMs在实时人机交互（HRI）中仍存在性能不足和延迟问题。


<details>
  <summary>Details</summary>
Motivation: 实时人类感知对有效的人机交互至关重要，但现有VLMs的高延迟限制了其实际应用。

Method: 构建HRIBench基准测试，包含1000个视觉问答问题，覆盖五个关键领域，并评估11种VLMs的性能与延迟。

Result: 当前VLMs在核心感知能力上表现不足，且缺乏适合实时部署的性能-延迟权衡。

Conclusion: 未来需开发更小、低延迟的VLMs以提升人类感知能力。
...
### [109] [X-SiT: Inherently Interpretable Surface Vision Transformers for Dementia Diagnosis](https://arxiv.org/abs/2506.20267)
*Fabian Bongratz,Tom Nuno Wolf,Jaume Gual Ramon,Christian Wachinger*

Main category: cs.GR

TL;DR: 论文提出了一种可解释的表面视觉变换器（X-SiT），用于医学图像分析，特别是脑部皮层数据的可视化与分类，展示了在阿尔茨海默病和额颞叶痴呆检测中的先进性能。


<details>
  <summary>Details</summary>
Motivation: 3D体积数据的复杂性使其难以直观解释，而皮层表面渲染提供了更易理解的脑部解剖表示。X-SiT旨在通过可解释的神经网络支持临床决策。

Method: X-SiT结合了原型表面补丁解码器，通过案例推理和空间对应的皮层原型对表面补丁嵌入进行分类。

Result: X-SiT在阿尔茨海默病和额颞叶痴呆检测中表现出色，同时提供了与已知疾病模式一致的原型，揭示了分类错误。
...
### [110] [DreamAnywhere: Object-Centric Panoramic 3D Scene Generation](https://arxiv.org/abs/2506.20367)
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到3D场景生成方法存在场景单一、视觉保真度低、缺乏场景理解等问题，DreamAnywhere旨在解决这些问题。
...
### [111] [EditP23: 3D Editing via Propagation of Image Prompts to Multi-View](https://arxiv.org/abs/2506.20652)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖文本提示或显式空间掩码，EditP23通过原始视图和用户编辑后的图像对实现更直观的编辑。
