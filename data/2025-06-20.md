...
Motivation: 老年患者在临床NLP资源中代表性不足，缺乏针对不良事件提取的高质量标注数据。

Method: 构建手动标注语料库，支持不连续和重叠实体标注，使用FlairNLP评估多种模型在不同粒度（细粒度、粗粒度、带否定的粗粒度）下的性能。

Result: BERT等模型在文档级粗粒度提取中表现优异（F1=0.943），但在细粒度任务（如F1=0.675）和罕见事件、复杂属性上表现显著下降。

Conclusion: 尽管整体性能良好，但细粒度不良事件提取仍具挑战性，数据集为未来研究提供了基准。
...
Method: BoostCD 分两阶段：阶段一分别进行约束和非约束解码生成弱预测；阶段二通过自回归增强模型结合两者生成最终预测。

Result: BoostCD 应用于封闭信息抽取任务（BoostIE），在分布内外均优于现有方法。

Conclusion: BoostCD 通过互补性利用约束和非约束解码的错误，显著提升了结构化输出的质量。
...
Result: 在三个模型架构和五个数据集上，CrEst表现优于基线，准确率提升26.86%，F1分数提高3.49%。

Conclusion: CrEst在无需人工标注的情况下有效提升LLM推理的可靠性，且在高噪声条件下表现稳健。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏针对多文档推理的评估基准，且传统基准创建成本高昂。

Method: 采用新颖的合成生成过程，基于结构化种子知识，通过LLM辅助编辑生成文档集和问答示例。

Result: MDBench对现有LLM和提示技术均构成显著挑战，且生成方法支持针对性分析和快速适应新挑战。

Conclusion: MDBench填补了多文档推理评估的空白，其生成方法高效且灵活，适用于未来模型改进。
...
Result: 专有LLMs（如GPT-4o和Gemma-2-27B）表现优于开源模型，Gemma-2-27B在F1分数上超越传统模型。提示策略和领域微调仍需改进。
...
<details>
  <summary>Details</summary>
Motivation: 探索LLM是否能够在不改变模型权重的情况下精确重建原始文本，并研究其潜在应用。

Method: 通过训练优化特殊记忆令牌的嵌入，使其在提示时能精确重建固定序列。

Result: 在英语和西班牙语数据集上，模型（包括Llama 3.1 8B）成功重建了长达240个令牌的序列。

Conclusion: LLM具备生成可逆嵌入的能力，可用于记忆检索、压缩和受控文本生成等应用。
...
<details>
  <summary>Details</summary>
Motivation: 社会隔离和孤独感近年显著增加，与自杀率密切相关，但未被美国国家暴力死亡报告系统记录，需通过NLP技术填补这一空白。

Method: 使用主题建模生成词典开发，结合监督学习分类器，分析执法和法医叙述。

Result: 开发出高质量分类器（F1平均0.86，准确率0.82），识别出1,198例慢性社会隔离相关自杀，特定人群（男性、同性恋、离婚者）风险更高。

Conclusion: 该方法可改善美国社会隔离与孤独感的监测和预防。
...
<details>
  <summary>Details</summary>
Motivation: 开放式长文本生成的评估具有挑战性，现有方法常忽略关键方面（如连贯性、风格或相关性）或受预训练数据偏差影响。

Method: 提出PrefBERT，基于两个多样化的长文本风格和质量评分数据集训练，为GRPO提供语义奖励反馈。

Result: PrefBERT在多句子和段落长度的响应中表现可靠，与人类偏好更一致，优于传统指标ROUGE-L和BERTScore。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs在现实中的广泛应用，事后遗忘能力变得至关重要，涉及隐私法规和纠正有害内容等需求。

Method: 通过实验研究学习时知识编码选择对遗忘事实知识效果的影响。

Result: 发现使用转述描述学习可提升遗忘性能，但遗忘文本块中的单个知识较困难。
...
Result: 在STAC和Molweni数据集上的实验表明，该方法有效解决歧义并显著优于SOTA基线。

Conclusion: DCM和CPO的结合显著提升了对话话语解析的性能，解决了歧义问题。
...
Method: 使用医学知识增强数据微调Qwen2.5-7B作为教师模型，通过多粒度注意力蒸馏机制生成可解释的软标签，并将知识迁移到轻量级BERT学生模型。
...
Conclusion: 两阶段框架有效解决开放域对话的一对多问题，为小模型优化提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 设计一种高效的分词器，减少分词数量（降低fertility）同时保持韩语的语言结构信息。

Method: 采用规则预分词方法，结合种子词汇和分支熵选择算法，增加平均分词长度。

Result: 实验显示，Thunder-Tok比BPE减少约10%的分词数量，推理速度提升10%，且不影响下游任务性能。

Conclusion: 基于语言学的分词方法能有效设计高效分词器，适用于语言模型。
...
<details>
  <summary>Details</summary>
Motivation: 探索语言模型中记忆的形成和遗忘机制，以理解信息如何在序列中保留和丢失。

Method: 在Mamba架构上应用结构化回忆任务，分析首因和近因效应，并通过消融实验和输入扰动验证发现。

Result: 发现长期记忆由稀疏通道支持，短期记忆受指数衰减调控，记忆分配受语义规律动态调节。

Conclusion: Mamba模型的记忆机制由多种因素共同作用，验证了其在大型语言模型中的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 联合国2012年提出的17个可持续发展目标（SDG）数据规模庞大且复杂，难以追踪进展，需要自动化工具支持。

Method: 研究比较了专有和开源LLM在单标签多类文本分类任务中的表现，并评估了零样本学习、少样本学习和微调等任务适应技术。

Result: 优化提示工程的小型模型表现可与OpenAI的GPT等大型模型媲美。

Conclusion: 提示工程优化的小型模型在SDG文本分类任务中具有潜力，可替代大型模型。
...
Conclusion: 验证了推理原型是LLMs泛化能力的基础，学习原型空间优于自然语言表示。
...
<details>
  <summary>Details</summary>
Motivation: 传统评估方法（如ROUGE和BERTScore）难以捕捉语义相似性，且现有LLM评估方法缺乏直观解释性，未能区分事实性和非事实性问题。

Method: MinosEval首先区分问题类型，对事实性问题采用自适应关键点评分策略，对非事实性问题采用实例感知的列表排序策略。

Result: 实验表明，MinosEval在多数据集上更符合人工标注，并提供更可解释的结果。

Conclusion: MinosEval通过针对性策略解决了开放性问题回答评估的挑战，提升了评估效果。
...
Method: 采用Graph RAG框架，结合知识图谱与检索增强生成，提出协作机制，优化模型性能。

Result: Xunzi-Qwen1.5-14B模型在关系抽取任务中表现最佳（F1=0.68），GraphRAG集成模型在开放域数据集上提升F1值11%。
...
Method: 采用混合检索策略、K-Means聚类和LLM生成多阶段答案的流程。
...
<details>
  <summary>Details</summary>
Motivation: 当前韩国法院判决的去标识化过程无法满足大规模处理需求，且法律对个人标识的定义模糊，不适合技术解决方案。

Method: 构建首个韩国法律数据集，系统化分类个人可识别信息（PII），并开发基于深度神经网络（DNN）的端到端去标识化流程。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在通用NLP任务中表现优异，但在关键领域（如临床试验招募）中的应用仍有限。LLMs的知识整合与推理能力有望解决传统方法的局限性。

Method: 通过调查分析，本文评估了LLM辅助方法在试验-患者匹配任务中的应用，并批判性地审查了现有基准、方法和评估框架。

Result: 研究发现，现有方法依赖专有模型和弱评估基准，限制了LLMs在临床研究中的广泛应用。

Conclusion: 本文为LLM在临床试验招募中的应用提供了首个系统性分析，指出了挑战与未来方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法难以区分LLMs的真实医学推理与表面模式，需更细粒度的临床理解评估。

Method: 构建MIMIC-IV ICU出院笔记数据集，生成真实和合成版本，评估五种LLMs在零样本和微调设置下的输入敏感性和下游推理。

Result: 零样本模型反事实推理更连贯，微调模型更稳定但对临床变化反应较弱；人口统计学因素对输出有微妙但持续影响。
...
Method: 创建SANSKRITI数据集，涵盖16个印度文化关键属性，并在主流语言模型上进行评估。

Result: 评估显示许多模型在处理文化细微差别时表现不佳，尤其在地区特定背景下。
...
<details>
  <summary>Details</summary>
Motivation: 印度语言在多模态和多语言摘要研究中的资源有限，COSMMIC旨在解决这一问题。

Method: 通过四种配置（纯文本、文本加评论、图片、三者结合）进行摘要和标题生成，并使用先进模型（如LLama3、GPT-4）评估。

Result: 数据集包含4,959篇文章和24,484条评论，支持九种印度语言，为NLG任务提供了有效配置。

Conclusion: COSMMIC通过整合多模态数据和用户反馈，推动了印度语言的NLP研究，并促进了包容性。
...
<details>
  <summary>Details</summary>
Motivation: 低资源语言在大型语言模型中的表现不佳，主要由于数据稀缺和预训练中的代表性不足，跨语言词汇对齐是关键挑战。

Method: 采用Low-Rank Adaptation（LoRA）和对比学习目标，针对模型早期层（如第2层）的嵌入进行微调。

Result: TLI显著提升了623个斯瓦希里语-英语词对的输出层词汇对齐，平均余弦相似度从0.3211提升至0.4113，且对未见词对也有良好泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在GUI代理中表现出潜力，但其存在系统性定位错误（幻觉），影响了可靠性。
...
Method: 引入RE-IMAGINE框架，基于因果阶梯理论生成不同层次的变体问题，评估LLMs的推理能力。

Result: LLMs在面对变体问题时表现下降，表明其部分依赖统计记忆。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅简单附加上下文无法确保生成内容基于外部上下文，因此需要一种更有效的方法来提升模型的生成能力。

Method: 提出CINGS方法，在训练时将相关上下文预置到响应中，但仅计算响应标记的损失并屏蔽上下文。

Result: CINGS在11个信息检索数据集上表现优于其他方法，且在视觉语言领域减少了幻觉并保持事实一致性。

Conclusion: CINGS通过改变模型对先验知识的依赖，隐式鼓励模型更依赖外部上下文，从而提升生成能力。
...
Method: SPARE框架通过单次标注和参考引导的步骤级评估，将每个解决步骤与参考解决方案对齐，并提供明确的评估理由。

Result: SPARE在数学推理、多跳组合问答和空间推理等领域的四个数据集上表现优异，推理性能提升且效率更高（运行时减少62%）。

Conclusion: SPARE在提升模型推理能力和效率方面具有显著优势，代码和模型已公开以促进研究和复现。
...
Method: EmoBi框架包括情感分析模块、基于情感的领域映射模块和双向动态交互模块，通过情感挖掘和领域映射提升检测效果，并设计验证机制确保准确性。

Result: 实验显示EmoBi在四个数据集上优于基线方法，F1分数在TroFi数据集上提升28.1%（夸张检测），在HYPO-L数据集上提升23.1%（隐喻检测）。

Conclusion: EmoBi通过情感引导和双向交互显著提升了夸张和隐喻检测效果，证明了其有效性和潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在生成可信回答时的常见问题，如遗漏明确答案、错误引用或拒绝可用证据。

Method: 使用GRPO方法，通过可验证的结果奖励训练模型，分阶段优化答案、引用和拒绝行为。

Result: 实验表明，推理增强模型在多个数据集上表现优于仅指令调优的模型，尤其在处理不可回答查询和生成引用充分的回答时。

Conclusion: 结合推理、分阶段优化和结果驱动的RL，可以构建更可靠和可验证的LLM。
...
Method: 提出RATTENTION，一种结合局部注意力和专门设计的线性注意力机制的方法，以捕获窗口外令牌的信息。

Result: 在3B和12B规模的预训练实验中，RATTENTION在性能和效率之间实现了更优的帕累托权衡，512窗口大小即可匹配全注意力模型的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现代语言模型通常公开权重但封闭训练数据，这导致数据难以获取。论文旨在解决从模型权重中近似训练数据的问题。

Method: 采用梯度基方法，从大型公共文本库中选择匹配度最高的数据，并通过原始模型和微调模型的权重恢复有用数据。

Result: 在AG News分类任务中，性能从65%提升至80%，接近专家基准88%；在MSMARCO文档的SFT模型中，困惑度从3.3降至2.3。

Conclusion: 该方法能有效从模型权重中恢复训练数据，显著提升模型性能，接近专家水平。
...
Conclusion: PredGen有效解决了LLMs在实时语音应用中的延迟问题，且计算开销可控。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注二元性别区分，而忽略了非二元性别的包容性，因此需要一种更全面的评估方法。

Method: 通过GIFI指标，从多个层次评估LLMs的性别包容性，包括代词测试、生成行为和认知行为等。
...
<details>
  <summary>Details</summary>
Motivation: 当前缺乏针对多模态科学声明验证的基准，SciVer旨在填补这一空白，推动模型在科学文献任务中的理解和推理能力。

Method: SciVer基于1113篇科学论文构建了3000个专家标注的示例，涵盖四种推理类型，并提供了支持证据。评估了21种先进多模态模型，包括o4-mini、Gemini-2.5-Flash等。

Result: 实验显示，现有模型与人类专家在SciVer上的表现存在显著差距。通过RAG和人工错误分析，揭示了开源模型的关键局限性。

Conclusion: SciVer为多模态科学声明验证提供了首个基准，揭示了当前模型的不足，为未来改进提供了重要方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本场景图解析方法在处理多句子描述时表现不佳，导致图结构碎片化和下游任务性能下降。

Method: 提出了DiscoSG-Refiner方法，使用两个小型PLM分阶段生成和优化场景图。

Result: DiscoSG-Refiner在SPICE指标上比基线方法提高了30%，推理速度比GPT-4快86倍。

Conclusion: DiscoSG-Refiner有效解决了多句子场景图解析问题，并在下游任务中表现优异。
...
Method: 引入WikiMixQA基准测试，包含1000个多选题，覆盖7个主题的4000个维基百科页面。评估了12个最先进的视觉语言模型。

Result: 专有模型在直接提供上下文时准确率约70%，但在长文档检索时性能显著下降。GPT-4-o是唯一准确率超过50%的模型，开源模型最高准确率仅为27%。
...
Method: 研究使用生成模型为葡萄牙语阅读理解生成MCQ，涵盖不同难度和课程相关叙事元素，通过专家评审和学生回答的心理测量分析评估质量。

Result: 模型生成的MCQ质量接近人工创作，但存在语义清晰度和干扰项设计问题。

Conclusion: 当前模型在葡萄牙语MCQ生成中表现良好，但需进一步改进语义清晰度和干扰项设计。
...
Method: 提出构建后悔数据集的工作流程，设计S-CDI指标识别最佳后悔表示层，RDS和GIC指标分析后悔神经元。

Result: 成功识别最佳后悔表示层，发现M形解耦模式，并将神经元分为三类功能组。

Conclusion: 提出的方法有效解决了后悔机制分析的挑战，为模型可靠性研究提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 探讨跨文化沟通误解的根源，特别是强化词的字面意义与语用因素（如礼貌和简洁）的作用。

Method: 通过三个实验和计算认知模型，分析英式和美式英语使用者对强化词的理解，模型结合了信息性、礼貌和表达成本。

Result: 跨文化差异源于字面意义和表达成本权重的不同，而非单纯的语义或礼貌规范。

Conclusion: 跨文化理解差异是字面意义与语用因素复杂交互的结果，挑战了单一因素的解释。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在生成任务中遵循指定概念顺序的能力，揭示其在组合泛化和指令遵循上的不足。

Method: 提出Ordered CommonGen基准，通过有序覆盖率评估LLMs的能力，并分析了36种LLMs的表现。

Result: LLMs虽能理解指令意图，但对特定顺序的偏好导致输出多样性低；最高指令遵循率仅75%。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在多任务中表现优异，但在推理一致性和鲁棒性方面仍有不足，需要改进。

Method: 提出CC-Learn框架，通过同类问题训练和复合目标优化（包括同类准确率、检索奖励和拒绝惩罚），利用强化学习提升推理一致性。

Result: 在ARC-Challenge和StrategyQA等基准测试中，CC-Learn显著优于预训练和SFT基线模型，提高了准确性和稳定性。

Conclusion: CC-Learn通过强化学习有效提升了大语言模型的推理一致性，为未来研究提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 性别包容的机器翻译需要保持源语言的性别模糊性，避免性别歧视和代表性伤害。

Method: 评估21种机器翻译系统在三种不同难度的翻译方向中对性别模糊的敏感性，并对实际观察到的性别中立策略进行分类和讨论。

Result: 大多数系统未能实现性别中立翻译，仅有少数系统在特定目标语言中采用性别中立策略。
...
<details>
  <summary>Details</summary>
Motivation: 由于大型VLMs在资源受限设备上部署困难，且不同架构的VLMs存在多样性，需要一种通用方法实现知识蒸馏。

Method: GenRecal引入Recalibrator模块，对齐和适应异构VLMs的特征表示，实现跨模型知识迁移。

Result: 实验表明，GenRecal显著提升基线性能，甚至超越大型开源和闭源VLMs。

Conclusion: GenRecal为异构VLMs的知识蒸馏提供了一种通用解决方案，具有广泛的应用潜力。
...
Method: 提出PhantomHunter，采用家族感知学习框架，捕获基础模型及其衍生模型的共享特征。
...
TL;DR: 本文介绍了Argus Inspection多模态基准和Eye of Panoptes框架，用于评估MLLMs在视觉细粒度感知和常识因果推理中的表现，发现其仍有较大提升空间。
...
<details>
  <summary>Details</summary>
Motivation: 猎鹰训练和狩猎需要确保猎鹰健康，传统疾病诊断方法效率不足。

Method: 采用ConvNeXt和EfficientNet混合模型，针对三种疾病（正常、肝病和曲霉病）进行分类。

Result: 混合模型在准确率、精确率、召回率和F1分数上表现优异，优于单一模型和传统方法。
...
<details>
  <summary>Details</summary>
Motivation: 开发一个透明且模块化的系统，能够结合视觉数据和自然语言查询来监测动物种群。
...
TL;DR: 论文提出GraphGSOcc模型，通过结合语义和几何图Transformer解决3D高斯泼溅（3DGS）方法中的特征聚合和边界模糊问题。
...
TL;DR: 论文提出了DAVID-X数据集和DAVID-XR1模型，用于可解释的AI生成视频检测，通过细粒度标注和自然语言解释提高检测的透明度和可信度。


<details>
  <summary>Details</summary>
Motivation: 随着AI生成视频在媒体平台上的普及，可靠区分合成内容与真实视频变得迫切且必要。现有方法仅将其视为二分类任务，缺乏对检测原因的详细解释。

Method: 引入DAVID-X数据集，提供详细的缺陷级时空标注和书面解释。基于此，开发DAVID-XR1视频语言模型，支持视觉推理链（缺陷分类、时空定位和自然语言解释）。

Result: 实验表明，通用骨干网络在DAVID-X数据集上微调并结合思维链蒸馏，能实现强泛化能力，适用于多种生成器和生成模式。

Conclusion: 可解释的检测方法为AI生成视频的可信识别提供了新方向，DAVID-XR1模型将检测过程从黑盒决策转变为透明可验证的诊断过程。
...
TL;DR: 本文综述了2020至2024年间基于深度学习的多智能体轨迹预测的最新进展，重点分析了ETH/UCY基准测试中的模型。


<details>
  <summary>Details</summary>
Motivation: 随着数据驱动方法在人类轨迹预测中的兴起，深入理解多智能体交互变得可能，这对自主导航和人群建模等领域具有重要意义。

Method: 文章对现有方法进行了分类，主要基于架构设计、输入表示和预测策略，并特别关注了ETH/UCY基准测试中的模型。

Result: 综述总结了多智能体轨迹预测领域的最新进展，并分析了不同方法的优缺点。
...
TL;DR: 论文提出ArchForms-4000数据集和ArchShapeNet模型，用于区分人类设计与机器生成的3D建筑形态，模型表现优于人类专家。


<details>
  <summary>Details</summary>
Motivation: 当代建筑设计需求复杂多样，生成工具快速生成初始概念和探索新形态的能力至关重要，但缺乏对人类设计与机器生成形态差异的客观分析。

Method: 构建ArchForms-4000数据集（2000个人类设计和2000个Evomass生成形态），提出ArchShapeNet（3D卷积神经网络，含显著模块），并进行对比实验。

Result: 模型在区分形态来源上表现优异（准确率94.29%，精确率96.2%，召回率98.51%），优于人类专家。
...
Method: 提出熵基自适应帧缓冲算法，并与MobileNetV2结合，优化处理流程。

Result: 在Raspberry Pi等设备上实现低于50ms的端到端推理延迟，检测准确率超过92%。

Conclusion: 系统具有可扩展性、低成本，且符合严格的数据隐私法规，适用于智能城市或嵌入式安全架构。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在图表到代码生成任务中表现不佳，需要精确的视觉理解和代码翻译能力。直接提示模型效果不理想。
...
TL;DR: 本文提出了一种名为BKSEF的框架，用于在CNN中动态选择最优的卷积核大小，通过信息理论、信号处理和学习理论的结合，显著提升模型精度并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 卷积核大小的选择在CNN设计中至关重要，但通常被忽视。本文旨在通过数学和实证方法优化这一选择，以平衡信息增益、计算效率和模型精度。

Method: 提出BKSEF框架，结合信息理论、信号处理和学习理论，动态确定每层的最优卷积核大小。

Result: 在多个数据集上，BKSEF引导的模型比传统3x3卷积核模型精度提升3.1%，计算量减少42.8%。实际案例验证了其在医疗图像分类和交通标志识别中的有效性。
...
Result: 方法准确率接近人工标注，成本节省50%，仅需人工验证5%的帧。

Conclusion: 该方法高效且经济，适用于多种零售场景。
...
Result: 仅使用一半视角即可达到与上限相当的精度，计算速度提升400倍，资源消耗减少50%以上。
...
Result: DPO-Kernels通过HT-SR保持强泛化边界，DETONATE基准包含10万张图像对。

Conclusion: DPO-Kernels在文本到图像模型中对齐性方面表现优异，DETONATE和AQI为未来研究提供了工具。
...
Method: 采用图像序列排列模拟多样位置关系，设计回放过滤机制优化策略利用。
...
TL;DR: 论文提出了一种针对医学图像扩散模型的频率校准重建误差（FCRE）方法，用于改进成员推断攻击（MIA），解决了现有方法在高频细节重建上的不足。
...
Method: 提出FCRE方法，通过专注于中频范围的重建误差，排除高频和低频区域，结合结构相似性指数评分确定成员关系。
...
TL;DR: 本文系统评估了Vision Transformer (ViT)及其混合模型在夸克-胶子喷注分类中的表现，发现其优于传统CNN方法。
...
Method: 使用模拟的2012年CMS公开数据，构建多通道喷注图像，评估ViT及ViT-CNN混合模型的性能。

Result: ViT及混合模型（如ViT+MaxViT和ViT+ConvNeXt）在F1分数、ROC-AUC和准确率上优于CNN基线。

Conclusion: ViT架构在喷注分类中表现出色，为后续研究提供了系统框架和基准。
...
TL;DR: 论文提出了基于LRCN和Transformer的模型，利用RGB触觉图像和GelSight传感器数据，显著提高了合规性预测的准确性。
...
Result: 模型在合规性估计上表现优于基线，但较硬物体的估计更具挑战性。

Conclusion: 提出的模型显著提升了合规性预测性能，但需进一步研究硬物体的估计问题。
...
TL;DR: PALETTE是一种端到端文本识别器，用于从多样化的历史地图中提取文本，通过超局部采样模块和位置嵌入技术提升性能，并结合SynthMap+合成数据训练。


<details>
  <summary>Details</summary>
Motivation: 历史地图中的文本包含丰富的地理、政治和文化信息，但缺乏有效的提取方法和训练数据，现有方法难以处理复杂背景和旋转文本。

Method: 提出PALETTE，采用超局部采样模块学习文本边界点和字符的局部特征，并结合超局部位置嵌入技术。同时开发SynthMap+合成数据用于训练。

Result: PALETTE在历史地图数据集上优于现有方法，尤其擅长处理长文本和旋转文本，已应用于6万张地图并生成1亿多文本标签。
...
<details>
  <summary>Details</summary>
Motivation: 传统风格迁移方法存在多种问题，且不同任务的框架不统一。StyleWallfacer旨在通过统一框架和新技术解决这些问题。

Method: 1. 基于BLIP和CLIP空间的语义风格注入方法；2. 基于人类反馈的数据增强策略；3. 训练自由的三重扩散过程。
...
Conclusion: 提出的方法有效解决了向量量化中的关键问题，显著提升了性能。
...
Method: 设计置信图协同模块（结合DINOv2和SAM），选择高质量正负提示点，并通过K-means聚类优化提示点，最终用于SAM分割。
...
Method: PDSA利用高维空间相关性校正特征分布，采用轻量级跨阶段结构描述符区分点相关性，并通过长距离建模增强结构同质性。
...
TL;DR: ReSeDis任务统一了大规模检索与像素级定位，解决了现有技术无法同时满足检索与定位的问题。


<details>
  <summary>Details</summary>
Motivation: 现有视觉搜索技术无法同时实现大规模检索和精确定位，ReSeDis旨在填补这一空白。

Method: 提出ReSeDis任务，结合检索与定位，并设计专用基准与评价指标。

Result: 通过零基线实验展示了未来研究的潜力。
...
TL;DR: 本文探讨了如何通过视觉上下文学习（VICL）训练通用模型用于视网膜光学相干断层扫描（OCT），以支持动态任务定义，并提出了针对VICL的评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有专用模型虽性能优异，但局限于预定义任务且开发成本高，而通用模型能动态适应任务，更具实用性。

Method: 采用视觉上下文学习（VICL）训练模型，使其能基于推理时的少量示例泛化到不同任务。

Result: 在多个视网膜OCT数据集上评估了先进的VICL方法，建立了首个基线，展示了其潜力与局限性。

Conclusion: VICL在OCT领域具有潜力，但仍需改进，代码已开源以促进研究和应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着视觉语言预训练模型（VLP）语义理解能力的提升，公开图像容易被搜索引擎等工具利用，因此需要保护用户隐私。
...
Result: 实验证明PSIC在多个下游任务中有效，同时保持图像压缩功能。

Conclusion: PSIC是一种即插即用的方法，可集成到现有图像压缩模型中，有效保护隐私。
...
TL;DR: 提出了一种基于扩散模型的两阶段融合网络（DM-FNet），用于多模态医学图像融合，解决了现有方法在细节捕获和跨模态特征交互上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有多模态医学图像融合方法在细节特征捕获和跨模态特征交互上表现不佳，导致融合图像质量不理想。
...
Method: 提出多轮DPO（MrDPO）方法，周期性更新参考模型，合并并重新初始化LoRA模块，结合真实字幕指导以稳定训练。

Result: MrDPO显著提升字幕准确性，错误率降低28%，7B参数的模型超越GPT-4o和Gemini-1.5-Pro。

Conclusion: 视频-SALMONN 2在视频字幕任务中表现优异，性能接近同规模的最先进模型。
...
Method: C-AFBiFPN结合了卷积特征增强模块（CFE）和创新的AFBiFPN网络，后者整合了BiFormer注意力机制以提升跨尺度特征融合的全局建模能力。
...
TL;DR: RA-NeRF是一种新方法，能够在复杂相机轨迹下预测高精度相机位姿，结合光度一致性和流驱动位姿调节，提升场景重建的鲁棒性。
...
TL;DR: 提出了一种名为RetroMem的COD架构，通过整合历史知识动态调节伪装模式感知和推理，显著提升模型性能。
...
Method: 采用两阶段训练范式（学习阶段和回忆阶段），设计DMA捕获多尺度信息，DMM和IPR利用历史知识重构推理。
...
TL;DR: 论文探讨了在半导体领域应用领域自适应（DA）技术的重要性，提出了一种改进的CycleGAN模型（DBACS），并在无监督和半监督设置下验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 半导体行业竞争激烈，上市时间和质量是关键因素。领域自适应技术可以减少人工标注和重新训练的需求，降低成本并提高效率。

Method: 提出了DBACS方法，基于CycleGAN模型并增加了额外的损失项，以提升性能。在无监督和半监督设置下测试了DA技术的有效性。

Result: 在真实电子显微镜图像数据集上验证了DBACS方法的有效性，证明了其在半导体领域的应用潜力。
...
TL;DR: MSNeRV是一种基于隐式神经表示（INR）的多尺度特征融合框架，用于视频压缩，解决了现有INR方法在细节密集和快速变化视频内容上的不足，并在压缩效率上超越了VTM-23.7。


<details>
  <summary>Details</summary>
Motivation: 现有INR方法在表示细节密集和快速变化的视频内容时表现不佳，主要原因是网络内部特征利用不足和缺乏视频特定设计。

Method: 提出MSNeRV框架，包括时域窗口增强时间一致性、分组视频处理、多尺度空间解码器和自适应损失函数，以及多尺度特征块。

Result: 在HEVC ClassB和UVG数据集上，MSNeRV在压缩效率和表示能力上优于现有INR方法，并超越VTM-23.7。
...
TL;DR: 论文提出BCRNet框架，通过贝塞尔曲线优化策略提升腹腔镜肝脏手术中的关键解剖标志检测精度。


<details>
  <summary>Details</summary>
Motivation: 腹腔镜肝脏手术中，准确识别关键解剖结构具有挑战性，AR系统结合MRI/CT与腹腔镜图像为手术导航提供了解决方案，但需精确检测曲线解剖标志。

Method: BCRNet框架包括多模态特征提取模块（MFE）、自适应曲线提案初始化（ACPI）和分层曲线优化机制（HCR），通过多阶段迭代优化贝塞尔曲线。

Result: 在L3D和P2ILF数据集上的实验表明，BCRNet优于现有方法，性能显著提升。

Conclusion: BCRNet通过贝塞尔曲线优化策略有效提升了腹腔镜手术中的标志检测精度，为手术导航提供了可靠支持。
...
Conclusion: ViMAT为工业装配任务的实时监控提供了灵活且高效的解决方案。
...
TL;DR: 论文探讨了面部微表情（MEs）的识别、检测和生成，提出了结合检测与识别的统一任务ME-STR，以及利用多模态大语言模型的ME-VQA任务。
...
Method: 提出了ME-STR（统一检测与识别）和ME-VQA（视觉问答）两种任务，利用多模态大语言模型增强分析能力。

Result: MEGC 2025挑战赛引入这两种任务，要求算法在测试集上运行并提交结果。

Conclusion: 通过整合任务和利用多模态模型，有望提升微表情分析的效率和准确性。
...
Result: 模型显著提升了特征表示质量，生成了更全面的场景表示，最终提高了矢量高精地图的预测准确性和质量。

Conclusion: MapFM通过多任务学习和基础模型结合，有效提升了在线矢量高精地图生成的性能，代码已开源。
...
TL;DR: OpenPath是一种基于预训练视觉语言模型（VLM）的开集主动学习方法，用于病理图像分类，通过任务特定提示和多样信息采样策略，显著提高了样本选择的纯度和信息量。


<details>
  <summary>Details</summary>
Motivation: 传统主动学习方法在开集场景下效率低下，且初始随机选择浪费标注成本。OpenPath旨在解决这些问题，提高病理图像分类的效率和性能。

Method: OpenPath结合任务特定提示和多样信息采样策略（DIS），包括原型候选选择（PIS）和熵引导随机采样（EGSS），以高效选择分布内（ID）样本。

Result: 在两个公开病理图像数据集上的实验表明，OpenPath显著提升了模型性能，优于现有开集主动学习方法。
...
TL;DR: 论文提出了一种新任务：视频中的开放世界目标计数，并介绍了CountVid模型和VideoCount数据集。


<details>
  <summary>Details</summary>
Motivation: 在拥挤场景中，由于遮挡和相似目标的存在，准确计数目标实例具有挑战性。

Method: 结合基于图像的计数模型和可提示的视频分割与跟踪模型，实现自动化计数。

Result: CountVid在VideoCount数据集上表现优异，显著优于基线方法。

Conclusion: CountVid和VideoCount为开放世界目标计数提供了有效解决方案，相关资源已开源。
...
Method: 使用表面法线估计指导展开过程，保持3D表面与2D纹理空间的几何一致性，无需真实UV标注，可自监督训练。

Result: 在环斑海豹和豹子数据集上，再识别准确率提升达5.4%。

Conclusion: 几何感知纹理映射方法有效提升了变形图案的再识别性能。
...
TL;DR: DDIS是一种基于扩散模型的数据自由图像合成方法，通过引入领域对齐指导和类对齐令牌，显著提升了合成图像的质量和对训练数据分布的准确性。
...
Method: DDIS利用文本到图像扩散模型作为图像先验，结合领域对齐指导（DAG）和类对齐令牌（CAT）优化合成图像与训练数据分布的对齐。

Result: 在PACS和ImageNet上的实验表明，DDIS优于现有方法，生成样本更准确地反映训练数据分布，实现SOTA性能。

Conclusion: DDIS通过扩散模型和领域对齐指导，有效解决了数据自由图像合成中的分布偏差问题，为数据不可用场景提供了高质量解决方案。
...
TL;DR: 论文提出了一种名为NERO的新型OOD评分机制，通过利用特征层的神经元级相关性来提升OOD检测性能，并在医学影像基准测试中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在医学影像领域，模型可靠性至关重要，而OOD检测是评估可靠性的重要指标。现有方法可能无法充分捕捉OOD多样性，因此需要更有效的解决方案。

Method: 提出NERO机制，通过聚类神经元级相关性形成类中心，并引入相关性距离度量新样本与中心的偏差。同时结合缩放相关性和特征范数优化性能。
...
<details>
  <summary>Details</summary>
Motivation: 3D AI生成内容领域复杂且难以普及，本文旨在通过教程形式简化流程，使其更易于研究人员和开发者使用。

Method: 使用Hunyuan3D 2.1系统，包括形状生成的Hunyuan3D-DiT和纹理合成的Hunyuan3D-Paint，涵盖数据处理、模型架构、训练策略和评估指标。

Result: 教程提供了完整的3D生成模型开发流程，适用于游戏、虚拟现实和工业设计。

Conclusion: 通过本教程，读者可以掌握开发或微调3D生成模型的能力，推动3D AIGC的普及。
...
TL;DR: MRG-LLM是一种新型多模态大语言模型，通过动态提示定制机制结合医学图像生成精准报告，性能优异。


<details>
  <summary>Details</summary>
Motivation: 医学影像报告生成具有挑战性，现有大语言模型与医学图像的有效结合仍需探索。

Method: 提出MRG-LLM，结合冻结的大语言模型与可学习的视觉编码器，通过视觉特征生成实例特定的动态提示。
...
TL;DR: 论文提出GenHOI框架，通过两阶段方法解决4D人-物交互（HOI）合成的挑战，包括泛化到未见物体和生成高质量4D HOI序列。


<details>
  <summary>Details</summary>
Motivation: 现有4D HOI数据集稀缺，限制了文本驱动的人体运动合成在4D HOI领域的进展。

Method: 1) 使用Object-AnchorNet从3D HOI数据重建稀疏3D HOI关键帧；2) 提出ContactDM模型，通过接触感知编码器和注意力机制插值生成密集4D HOI序列。

Result: 在OMOMO和3D-FUTURE数据集上取得最优结果，展示了对未见物体的强泛化能力和高质量4D HOI生成。

Conclusion: GenHOI框架有效减少对大规模4D HOI数据的依赖，实现了高质量的4D HOI合成。
...
<details>
  <summary>Details</summary>
Motivation: 研究阴影去除技术，通过挑战赛推动算法在复杂场景中的表现。

Method: 使用WSRD+数据集评估，模拟自阴影和投射阴影的交互。

Result: 17支团队提交了解决方案，评估分为重建保真度和视觉感知两个轨道。

Conclusion: 挑战赛展示了阴影去除技术的进展，尤其是在复杂场景中的表现。
...
TL;DR: CLAIM框架通过临床知识引导的LGE增强和扩散生成技术，合成多样且解剖学一致的心肌疤痕图像，并结合联合训练策略提升分割性能。


<details>
  <summary>Details</summary>
Motivation: LGE心脏MRI图像稀缺且质量不一，限制了深度学习模型在心肌疤痕分割中的鲁棒性发展。

Method: CLAIM框架包含SMILE模块（临床知识引导的疤痕生成）和联合训练策略，结合AHA 17段模型生成解剖学一致的疤痕图像，并优化分割网络。
...
TL;DR: RaCalNet提出了一种无需密集LiDAR监督的毫米波雷达深度估计框架，通过稀疏LiDAR监督学习精确深度，显著降低监督密度。
...
Conclusion: RaCalNet证明了稀疏监督的可行性，提升了深度估计的结构一致性和细节保留能力。
...
Conclusion: WinWinLay通过理论分析和创新策略，显著提升了布局到图像生成的质量和可控性。
...
<details>
  <summary>Details</summary>
Motivation: 通过统一的多模态表示和高效的训练方法，提升跨模态任务的处理能力。

Method: 基于3D因果变分自编码器空间，采用双路径时空融合构建统一表示，结合自回归建模和流匹配技术。
...
TL;DR: 提出了一种参数高效的半监督分割框架，用于0.3米高分辨率遥感影像，结合SAM2知识和FreqWeaver Adapter，显著提升细粒度细节建模能力。


<details>
  <summary>Details</summary>
Motivation: 高分辨率土地覆盖分类面临像素级标注成本高、尺度变化大和大规模视觉模型适应性差等挑战，现有方法多依赖标注数据且仅适用于1米分辨率。

Method: 采用半监督学习框架，结合SAM2知识和FreqWeaver Adapter，仅占用总参数的5.96%，高效利用未标注数据。

Result: 在结构一致性上表现优异，比现有参数高效方法提升1.78%，比最先进高分辨率遥感分割方法提升3.44%。

Conclusion: 该方法在高分辨率遥感影像分割中具有显著优势，参数效率高且适应弱监督场景。
...
<details>
  <summary>Details</summary>
Motivation: 当前定量结构模型（QSM）方法依赖高质量点云数据且需要复杂预处理，限制了其在大规模应用中的实用性。

Method: 通过创新的基于图的流程，整合了树木分割、叶木分离和3D骨架重建，实现了端到端处理。

Result: 在多种条件下（如叶茂和叶落场景、不同空间尺度和数据源）表现出色，相对误差低至20%-30%。
...
Method: 通过Cross-Frame Retrieval（CFR）模块和Consistency-LoRA（C-LoRA）学习时间一致性，再训练Detail-LoRA（D-LoRA）增强细节。

Result: DLoRAL在精度和速度上均表现出色。

Conclusion: DLoRAL通过双LoRA学习范式有效解决了视频超分辨率中细节和时间一致性的平衡问题。
...
TL;DR: 提出了一种名为M2M-Reg的新框架，用于多模态图像配准，通过单模态相似性训练模型，解决了传统方法在高度异质模态配准中的问题。
...
Method: M2M-Reg框架通过单模态相似性训练多模态配准模型，并引入GradCyCon正则化器促进微分同胚。支持半监督设置，无需真实变换或分割掩码。
...
TL;DR: 提出了一种无需重建的实时3D物体检测框架，结合预训练视觉模型和CLIP，实现高效内存和开放词汇语义检测。
...
Conclusion: 无需重建的框架在多种场景中表现优异，适用于实时应用。
...
TL;DR: HOIDiNi是一个基于文本驱动的扩散框架，用于生成逼真且合理的人-物交互（HOI）。它通过Diffusion Noise Optimization（DNO）直接优化预训练扩散模型的噪声空间，实现了真实性与物理准确性的平衡。
...
Method: HOIDiNi将问题分为两个阶段：以物体为中心的阶段（选择手-物接触位置）和以人为中心的阶段（优化全身动作）。通过DNO优化噪声空间。
...
TL;DR: 研究发现，多模态大语言模型（MLLMs）在视觉质量与性能之间存在矛盾，图像偏离人类感知的保真度时性能反而可能提升。为此，作者提出了一种轻量级适配模块VQ-TTT，动态调整输入图像以提升任务性能。


<details>
  <summary>Details</summary>
Motivation: 探讨视觉输入质量对MLLMs性能的影响，揭示现有模型对视觉质量的非直观偏好。

Method: 通过控制图像退化与风格变化，研究MLLMs的响应；提出VQ-TTT模块，动态调整输入图像以适配模型偏好。

Result: VQ-TTT显著提升了MLLMs在多个数据集上的平均准确率，无需额外模型或数据。

Conclusion: 研究重新定义了MLLMs的“更好”视觉输入，强调需要自适应而非“干净”的图像。
...
TL;DR: ViMaR是一个两阶段推理框架，通过结合时间差分价值模型和边缘感知奖励调整，提升视觉语言模型的效率和输出保真度。
...
Method: 第一阶段选择最高价值候选描述，第二阶段选择性优化弱视觉基础部分，并引入边缘惩罚机制。

Result: ViMaR生成更可靠、准确、详细的描述，速度提升4倍，并能跨模型泛化。
...
TL;DR: 提出了一种联合估计反照率并生成重光照输出的单阶段方法，利用视频扩散模型的生成能力，显著提升了场景理解和光照效果的真实性。
...
Result: 模型在多样场景中表现出强泛化能力，在视觉真实性和时间一致性上超越先前方法。

Conclusion: 该方法通过联合优化和生成模型的应用，显著提升了重光照任务的效果，适用于复杂光照和材质场景。
...
Method: 开发了一个高效的工具箱，用于收集、预处理和标注视频，包含位置、场景、天气、人群密度、字幕和相机轨迹等信息。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型推理速度慢且计算成本高，现有方法依赖固定启发式规则，效果有限或泛化性差。
...
### [117] [Empirical Studies of Large Scale Environment Scanning by Consumer Electronics](https://arxiv.org/abs/2506.14771)
*Mengyuan Wang,Yang Liu,Haopeng Wang,Haiwei Dong,Abdulmotaleb El Saddik*

Main category: eess.IV

TL;DR: 本文评估了Matterport Pro3在大型环境重建中的表现，展示了其高密度点云和高对齐精度的优势。


<details>
  <summary>Details</summary>
Motivation: 研究Matterport Pro3在大型建筑扫描中的实际效果，探索其性能提升和局限性。

Method: 通过1,099个扫描点对六层建筑（17,567平方米）进行详细扫描，并与iPhone设备对比。

Result: Pro3生成1,877,324个点云，RMSE为0.0118米，优于iPhone的506,961点和0.0408米C2C误差。
...
### [118] [Deploying and Evaluating Multiple Deep Learning Models on Edge Devices for Diabetic Retinopathy Detection](https://arxiv.org/abs/2506.14834)
*Akwasi Asare,Dennis Agyemanh Nana Gookyi,Derrick Boateng,Fortunatus Aabangbio Wulnye*

Main category: eess.IV

TL;DR: 该研究利用Edge Impulse部署深度学习模型在边缘设备上实现实时糖尿病视网膜病变（DR）检测，通过多种CNN模型优化，MobileNet准确率达96.45%，SqueezeNet在GPU上仅17ms延迟。


<details>
  <summary>Details</summary>
Motivation: 传统DR诊断依赖人工检查，耗时耗力，研究旨在通过边缘AI技术提供高效、低成本的解决方案。

Method: 使用Kaggle EyePACS数据集，预处理后训练多种CNN模型（MobileNet、ShuffleNet等），优化为TensorFlowLite并量化，评估不同硬件性能。

Result: MobileNet准确率96.45%，SqueezeNet模型小（176KB）、延迟低（17ms），适合实时应用；其他模型在资源效率上表现优异。

Conclusion: 边缘AI技术为DR早期检测提供了可扩展、低成本的解决方案，尤其适合资源有限地区。
...
<details>
  <summary>Details</summary>
Motivation: 解决T2加权MRI图像中前列腺分割因读者差异和跨站点数据异质性导致的自动分割挑战。

Method: 比较UNETR、SwinUNETR和3D UNet模型，采用三种训练策略（单队列、5折交叉验证混合队列、基于腺体大小的数据集），并通过Optuna优化超参数。

Result: SwinUNETR在跨读者和跨站点数据中表现最佳，Dice分数最高提升5个百分点，尤其在腺体大小较大的子集中表现突出。
...
### [120] [Foundation Artificial Intelligence Models for Health Recognition Using Face Photographs (FAHR-Face)](https://arxiv.org/abs/2506.14909)
...
TL;DR: FAHR-Face是一个基于4000万张面部图像训练的基础模型，用于生物年龄估计（FAHR-FaceAge）和生存风险预测（FAHR-FaceSurvival）。模型在公开数据集和癌症患者数据中表现出色，并展示了跨年龄、性别、种族和癌症亚组的泛化能力。
...
Method: FAHR-FaceAge通过两阶段年龄平衡微调训练，FAHR-FaceSurvival在癌症患者照片上微调。测试了模型的鲁棒性和独立性，并在独立癌症患者数据集中进行临床验证。

Result: FAHR-FaceAge在公开数据集上平均绝对误差为5.1年，优于基准模型。FAHR-FaceSurvival能稳健预测死亡率，最高风险组死亡率是最低风险组的三倍以上。两种模型提供了互补的预后信息。

Conclusion: 单一基础模型可生成廉价、可扩展的面部生物标志物，捕捉生物衰老和疾病相关死亡风险，并能利用小规模临床数据集有效训练。
...
<details>
  <summary>Details</summary>
Motivation: 解剖树在临床诊断和治疗规划中至关重要，但现有方法难以准确表示其复杂多变的拓扑和几何结构。

Method: 开发了RvNN，利用血管的层次结构学习分支连接性和几何特征的低维流形，通过采样生成新血管几何。
...
TL;DR: 提出了一种基于Transformer的Mixture-of-Experts框架，用于整合多模态MRI和临床数据，显著提升神经退行性疾病的诊断准确性。
...
Result: 在多模态数据集上验证，准确率达82.47%，比基线方法提升超过10%。

Conclusion: 该框架通过多模态学习显著提升了神经退行性疾病的诊断性能，适用于真实临床场景。
...
### [123] [Classification of Multi-Parametric Body MRI Series Using Deep Learning](https://arxiv.org/abs/2506.15182)
...
<details>
  <summary>Details</summary>
Motivation: 解决DICOM头信息错误导致的MRI序列分类问题，以提高放射科医生的工作效率。

Method: 使用ResNet、EfficientNet和DenseNet训练分类器，比较性能，并研究不同训练数据量和外部数据集的表现。

Result: DenseNet-121在内部数据集上F1-score为0.966，准确率为0.972；在外部数据集上准确率分别为0.872和0.810。
...
### [124] [Privacy-Preserving Chest X-ray Classification in Latent Space with Homomorphically Encrypted Neural Inference](https://arxiv.org/abs/2506.15258)
*Jonghun Kim,Gyeongdeok Jo,Shinyoung Ra,Hyunjin Park*

Main category: eess.IV

TL;DR: 提出了一种基于VQGAN和HE的医疗图像隐私保护推理框架，通过压缩图像和多项式近似激活函数降低计算负担，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 医疗图像数据包含敏感信息，需要隐私保护，但HE推理计算成本高，特别是对大图像。
...
### [125] [FedWSIDD: Federated Whole Slide Image Classification via Dataset Distillation](https://arxiv.org/abs/2506.15365)
*Haolong Jin,Shenglin Liu,Cong Cong,Qingmin Feng,Yongzhi Liu,Lina Huang,Yingzi Hu*

Main category: eess.IV

TL;DR: FedWSIDD是一种新的联邦学习范式，通过数据集蒸馏（DD）传输合成切片，解决了WSI分类中的计算资源异构性和隐私问题。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在WSI分类中面临计算资源异构性和隐私问题，需要一种既能保护数据隐私又能提升分类性能的方法。

Method: 提出FedWSIDD，利用DD生成合成切片并传输，服务器聚合切片并分发，客户端结合DD算法和染色归一化生成合成切片。

Result: 在CAMELYON16和CAMELYON17等任务上，FedWSIDD提升了分类性能，支持异构模型，同时保护隐私。
...
### [126] [A Real-time Endoscopic Image Denoising System](https://arxiv.org/abs/2506.15395)
...
<details>
  <summary>Details</summary>
Motivation: 单次使用内窥镜的小型化设计虽带来诸多优势，但受限于传感器尺寸和电路设计，图像噪声问题显著，影响诊断效果。

Method: 建立了模拟图像传感器的噪声模型，结合传统图像处理算法和基于学习的先进技术，提出混合去噪系统。

Result: 实验表明，该方法显著降低噪声，PSNR从21.16提升至33.05，且在FPGA平台上实现实时性能。

Conclusion: 混合去噪系统有效解决了小型传感器内窥镜的图像噪声问题，提升了诊断质量。
...
### [127] [Advanced cervical cancer classification: enhancing pap smear images with hybrid PMD Filter-CLAHE](https://arxiv.org/abs/2506.15489)
...
<details>
  <summary>Details</summary>
Motivation: 宫颈癌早期检测至关重要，但CNN性能受图像质量影响。研究旨在通过预处理技术提升图像质量和分类性能。
...
### [128] [Automated MRI Tumor Segmentation using hybrid U-Net with Transformer and Efficient Attention](https://arxiv.org/abs/2506.15562)
*Syed Haider Ali,Asrar Ahmad,Muhammad Ali,Asifullah Khan,Muhammad Shahban,Nadeem Shaukat*

Main category: eess.IV

TL;DR: 该研究提出了一种混合UNet-Transformer模型，用于本地医院MRI数据集的肿瘤分割，通过高效的数据处理和模型优化，实现了竞争性的分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有AI分割模型通常基于大型公共数据集，缺乏本地患者群体的异质性，因此需要开发适用于本地数据的模型以优化肿瘤放疗计划。
...
### [129] [Detecting Narrative Shifts through Persistent Structures: A Topological Analysis of Media Discourse](https://arxiv.org/abs/2506.14836)
*Mark M. Bailey,Mark I. Heiligman*

Main category: cs.SI

TL;DR: 该研究提出了一种基于持续同调的拓扑框架，用于检测媒体叙事中的结构性变化，通过分析重大事件前后的名词共现图，捕捉语义中断和叙事波动。


<details>
  <summary>Details</summary>
Motivation: 研究旨在识别全球事件如何重塑公共话语，特别是在重大事件发生时，媒体叙事的结构和连贯性如何突然重组。

Method: 利用国际新闻文章构建每日名词共现图，通过Vietoris-Rips过滤生成持续图，计算Wasserstein距离和持续熵来量化语义变化。

Result: 结果显示，重大事件与H0和H1的急剧峰值相关，表明叙事结构的突然变化。H0变化通常先于H1，但俄罗斯入侵乌克兰事件中H1领先。

Conclusion: 持续同调提供了一种无监督的数学方法，可实时检测公共话语中的转折点和方向性变化，适用于危机、抗议和信息冲击的研究。
...
### [130] [Factorized RVQ-GAN For Disentangled Speech Tokenization](https://arxiv.org/abs/2506.15456)
...
TL;DR: HAC是一种分层音频编解码器，通过单一模型将瓶颈分解为声学、音素和词汇三个层次，利用知识蒸馏目标实现分层表示，在英语和多语言数据上表现优异。


<details>
  <summary>Details</summary>
Motivation: 提出一种统一的神经语音编解码器，以分层方式捕捉语音的声学、音素和词汇信息，为下游任务提供更丰富的表示。

Method: HAC利用HuBERT和LaBSE的知识蒸馏目标，分别提取音素和词汇信息，并通过分层瓶颈实现解耦表示。

Result: 实验表明HAC的分层瓶颈能解耦音素和词汇信息，在自然度和重建质量上优于单层基线。

Conclusion: HAC作为一种统一的离散语音表示，有望连接声学细节和词汇语义，支持下游语音生成和理解任务。
...
### [131] [Embodied Web Agents: Bridging Physical-Digital Realms for Integrated Agent Intelligence](https://arxiv.org/abs/2506.15677)
*Yining Hong,Rui Sun,Bingxuan Li,Xingcheng Yao,Maxine Wu,Alexander Chien,Da Yin,Ying Nian Wu,Zhecan James Wang,Kai-Wei Chang*

Main category: cs.AI

TL;DR: 论文提出了一种新型AI代理范式——Embodied Web Agents，旨在融合物理世界交互与网络规模推理，并通过统一仿真平台和多样化任务基准评估其性能。
...
Method: 开发了Embodied Web Agents任务环境和仿真平台，整合3D环境与网络接口，并构建了包含多种任务的基准测试。

Result: 实验显示现有AI系统与人类能力存在显著差距，揭示了跨领域智能的挑战与机遇。

Conclusion: 论文为融合物理与数字智能的AI代理提供了新范式，并公开了数据集和代码以推动研究。
...
TL;DR: 本文提出了一种定量框架，用于评估大型语言模型（LLM）在多项选择调查中模拟人类行为的偏差，并发现模型在模拟特定人群时存在显著不匹配。
...
Method: 采用假设检验方法，构建定量框架评估LLM模拟行为与真实人类行为的差异。

Result: 研究发现，测试的语言模型在模拟争议性问题时，对不同种族、年龄和收入人群的表现不佳。

Conclusion: LLM在社会科学研究中的应用需要更谨慎的方法，而非简单模拟人类行为。
...
### [133] [Towards Perception-based Collision Avoidance for UAVs when Guiding the Visually Impaired](https://arxiv.org/abs/2506.14857)
...
### [134] [Robust Instant Policy: Leveraging Student's t-Regression Model for Robust In-context Imitation Learning of Robot Manipulation](https://arxiv.org/abs/2506.15157)
*Hanbit Oh,Andrea M. Salcedo-Vázquez,Ixchel G. Ramirez-Alpizar,Yukiyasu Domae*

Main category: cs.RO

TL;DR: 提出了一种新的鲁棒上下文模仿学习算法（RIP），通过利用Student's t回归模型来减少LLM生成的幻觉轨迹，从而提高任务成功率。
...
Method: 使用Student's t分布聚合LLM生成的多个候选轨迹，以忽略异常值（幻觉）。

Result: 在模拟和真实环境中，RIP显著优于现有方法，任务成功率提高至少26%。

Conclusion: RIP通过鲁棒聚合方法有效减少了幻觉问题，提升了模仿学习的可靠性。
...
### [135] [MCOO-SLAM: A Multi-Camera Omnidirectional Object SLAM System](https://arxiv.org/abs/2506.15402)
...
Result: 实验表明，MCOO-SLAM能实现精准定位和可扩展的物体级建图，对遮挡、姿态变化和环境复杂性具有更强鲁棒性。
...
### [136] [Particle-Grid Neural Dynamics for Learning Deformable Object Models from RGB-D Videos](https://arxiv.org/abs/2506.15680)
...
Method: 采用粒子-网格混合表示，粒子表示物体形状，空间网格确保空间连续性，结合高斯渲染生成3D动作视频。

Result: 模型能够从稀疏视角的RGB-D记录中学习多种物体的动态行为，并在类别级别泛化到未见实例，性能优于现有方法。

Conclusion: 该框架为可变形物体的学习型数字孪生提供了有效解决方案，并支持基于模型的规划任务。
...
### [137] [ETS: Open Vocabulary Electroencephalography-To-Text Decoding and Sentiment Classification](https://arxiv.org/abs/2506.14783)
...
### [138] [Assembly of Experts: Linear-time construction of the Chimera LLM variants with emergent and adaptable behaviors](https://arxiv.org/abs/2506.14794)
...
<details>
  <summary>Details</summary>
Motivation: 预训练大型语言模型的计算成本极高（10^13-10^15 FLOPs/8位权重），因此需要更高效的方法利用现有模型资源。

Method: 通过线性时间内的权重张量插值，从父模型中生成子模型，增强或抑制语义特征。

Result: 生成的子模型功能性强，行为特征可渐变或突变。DeepSeek R1T“Chimera”模型在继承R1部分特性的同时，减少了40%的输出令牌，接近V3速度。

Conclusion: AoE方法无需微调或蒸馏即可生成高性能子模型，展示了紧凑且有序的推理能力。
...
### [139] [Revisiting Reinforcement Learning for LLM Reasoning from A Cross-Domain Perspective](https://arxiv.org/abs/2506.14965)
...
TL;DR: 论文介绍了Guru，一个包含92K可验证示例的RL推理语料库，涵盖六个推理领域，并通过系统实验揭示了RL在不同领域的适用性差异。基于Guru训练的两个模型在多个任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有RL研究主要集中于数学和代码领域，缺乏对其他推理领域的广泛适用性研究，且缺乏可靠、可扩展的奖励信号。

Method: 构建Guru语料库，包含六个推理领域的92K示例，通过领域特定的奖励设计、去重和过滤确保可靠性。基于Guru重新评估RL对LLM推理的影响，并训练Guru-7B和Guru-32B模型。

Result: Guru-7B和Guru-32B在17个任务评估中分别优于基线7.9%和6.7%，且在复杂任务上显著提升Pass@k性能。

Conclusion: RL在不同领域的适用性存在显著差异，Guru语料库和模型为通用推理提供了有效工具。
...
### [140] [Optimal Embedding Learning Rate in LLMs: The Effect of Vocabulary Size](https://arxiv.org/abs/2506.15025)
*Soufiane Hayou,Liyuan Liu*

Main category: cs.LG

TL;DR: 论文分析了词汇量对大型语言模型训练动态的影响，揭示了词汇量增加时训练动态在μP和LV两种机制之间的变化，并提出了新的嵌入学习率缩放规则。


<details>
  <summary>Details</summary>
Motivation: 研究词汇量对μP理论的影响，解决其在大型语言模型中的局限性。

Method: 理论分析词汇量对训练动态的影响，并通过实验验证提出的缩放规则。

Result: 发现词汇量增加时训练动态介于μP和LV机制之间，提出了嵌入学习率与隐藏学习率的最优比例应为Θ(√width)。

Conclusion: 提出的缩放规则在实验中表现良好，为大型语言模型的高效训练提供了新思路。
...
### [141] [When and How Unlabeled Data Provably Improve In-Context Learning](https://arxiv.org/abs/2506.15329)
...
<details>
  <summary>Details</summary>
Motivation: 探讨上下文学习在标签缺失或错误时的表现，理解模型如何利用未标记数据。

Method: 使用二元高斯混合模型（GMM）生成演示数据，分析单层线性注意力模型和多层/循环Transformer的表现。

Result: 单层模型无法利用未标记数据，而多层或循环模型能有效利用，且其性能随深度指数提升。
...
### [142] [Capturing Polysemanticity with PRISM: A Multi-Concept Feature Description Framework](https://arxiv.org/abs/2506.15538)
*Laura Kopf,Nils Feldhus,Kirill Bykov,Philine Lou Bommer,Anna Hedström,Marina M. -C. Höhne,Oliver Eberle*

Main category: cs.LG

TL;DR: PRISM是一种新框架，用于捕捉神经网络特征的复杂性，解决了现有方法在鲁棒性和单义性假设上的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有特征描述方法假设神经元仅编码单一概念（单义性），而实际上神经元通常是多义的，这限制了描述的准确性和表达能力。

Method: 提出PRISM框架，为多义和单义特征提供更细致的描述，应用于语言模型并通过基准测试验证。

Result: PRISM生成的特征描述更准确和忠实，提升了描述质量和捕捉多义概念的能力。

Conclusion: PRISM通过捕捉特征的复杂性，显著改进了神经网络特征描述的准确性和表达能力。
...
### [143] [LoX: Low-Rank Extrapolation Robustifies LLM Safety Against Fine-tuning](https://arxiv.org/abs/2506.15606)
...
Method: 提出Low-Rank Extrapolation (LoX)，通过外推对齐LLM的安全子空间来增强安全性。

Result: 实验表明，LoX在面对良性或恶意微调攻击时，攻击成功率降低了11%至54%。

Conclusion: LoX通过将LLM参数移动到更平坦的区域，显著提升了安全鲁棒性，同时保持了模型对新任务的适应性。
...
### [144] [AutoRule: Reasoning Chain-of-thought Extracted Rule-based Rewards Improve Preference Learning](https://arxiv.org/abs/2506.15651)
*Tevin Wang,Chenyan Xiong*

Main category: cs.LG

TL;DR: AutoRule是一种自动化方法，通过从用户偏好反馈中提取规则并转化为规则奖励，提升强化学习效果。实验显示其在多个任务中优于基线方法。
...
Method: AutoRule分三阶段：通过推理模型解释用户偏好，从解释链中提取候选规则，合成统一规则集，并利用语言模型验证规则满足度作为辅助奖励。

Result: 实验表明，AutoRule在AlpacaEval2.0和MT-Bench上分别提升28.6%和6.1%的性能，且规则与数据集偏好一致，减少了奖励滥用。

Conclusion: AutoRule能自动提取有效规则，提升强化学习性能，且规则具有数据集特异性，代码已开源。
...
### [145] [PIPE: Physics-Informed Position Encoding for Alignment of Satellite Images and Time Series](https://arxiv.org/abs/2506.14786)
*Haobo Li,Eunseo Jung,Zixin Chen,Zhaowei Wang,Yueya Wang,Huamin Qu,Alexis Kai Hon Lau*

Main category: cs.LG

TL;DR: 论文提出了一种轻量级方法PIPE，通过物理信息位置编码提升多模态时间序列预测的准确性，特别是在卫星图像数据中。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注文本数据，忽略了视觉数据中的物理信息，如卫星图像的时空背景。

Method: PIPE通过物理信息位置索引和变频率位置编码机制，将物理信息嵌入视觉语言模型。

Result: 在最大开源卫星图像数据集上，PIPE在台风强度预测中比现有方法提升了12%。

Conclusion: PIPE在多模态对齐和预测准确性上表现优异，为气候科学等领域提供了新工具。
...
### [146] [Dense SAE Latents Are Features, Not Bugs](https://arxiv.org/abs/2506.15679)
*Xiaoqing Sun,Alessandro Stolfo,Joshua Engels,Ben Wu,Senthooran Rajamanoharan,Mrinmaya Sachan,Max Tegmark*

Main category: cs.LG

TL;DR: 研究发现稀疏自编码器（SAEs）中的密集潜在特征并非训练噪声，而是具有功能性的模型表示。


<details>
  <summary>Details</summary>
Motivation: 探讨SAEs中密集潜在特征的几何、功能和起源，以验证其是否为训练过程中的不良产物。

Method: 系统分析密集潜在特征的几何结构、功能分类及其在模型各层的演变。

Result: 密集潜在特征形成反极对，重构残差流中的特定方向，并在不同层中表现出从结构到语义再到输出信号的演变。
...
### [147] [Reinforcing VLMs to Use Tools for Detailed Visual Reasoning Under Resource Constraints](https://arxiv.org/abs/2506.14821)
...
Method: 采用Group Relative Policy Optimization (GRPO)训练小规模模型，结合简单奖励结构、简化工具调用接口、为工具调用结果分配额外token，以及侧重视觉难度高的训练数据。

Result: 在部分视觉问答任务上，该方法优于同类基线模型，归功于从外部工具获取的详细视觉信息。
...
### [148] [CACTUS as a Reliable Tool for Early Classification of Age-related Macular Degeneration](https://arxiv.org/abs/2506.14843)
...
TL;DR: 论文提出了一种名为CACTUS的工具，用于改进AMD（年龄相关性黄斑变性）的分期分类，解决了医疗数据不完整和模型透明度不足的问题。


<details>
  <summary>Details</summary>
Motivation: 医疗数据通常不完整且有限，影响机器学习模型的性能。AMD的早期诊断至关重要，但缺乏有效的治疗方法。现有分类方法需要综合考虑多种因素。

Method: 开发了CACTUS工具，提供可解释性和灵活性，优于标准ML模型。通过识别关键因素并消除不相关或有偏数据，改进决策。

Result: CACTUS在AMD分期分类中表现优于标准ML模型，并能识别重要特征，与现有医学知识对比验证。

Conclusion: CACTUS为AMD诊断提供了一种透明且高效的分类工具，有助于临床决策和减少偏差。
...
### [149] [Pixel-level Certified Explanations via Randomized Smoothing](https://arxiv.org/abs/2506.15499)
...
<details>
  <summary>Details</summary>
Motivation: 现有的后验归因方法在输入扰动下表现脆弱，影响了其可信度，因此需要严格的鲁棒性保证。

Method: 采用随机平滑技术，将归因图稀疏化和平滑化，并将其重新表述为分割问题，以认证每个像素的重要性。
...
### [150] [Identifying economic narratives in large text corpora -- An integrated approach using Large Language Models](https://arxiv.org/abs/2506.15041)
*Tobias Schmidt,Kai-Robin Lange,Matthias Reccius,Henrik Müller,Michael Roos,Carsten Jentsch*

Main category: econ.GN

TL;DR: 论文探讨了利用大型语言模型（如GPT-4）提取经济叙事的潜力，并与传统方法对比，发现其虽有效但仍不及专家水平。


<details>
  <summary>Details</summary>
Motivation: 近年来对经济叙事的兴趣增加，但现有方法缺乏深层语义理解，无法区分经济叙事提取与传统任务。
...
### [151] [Omnidirectional Video Super-Resolution using Deep Learning](https://arxiv.org/abs/2506.14803)
*Arbind Agrahari Baniya,Tsz-Kwan Lee,Peter W. Eklund,Sunil Aryal*

Main category: cs.MM

TL;DR: 论文提出了一种针对360°视频的超分辨率方法S3PO，解决了传统VSR模型在360°视频中的失真问题，并创建了新的数据集360VDS。


<details>
  <summary>Details</summary>
Motivation: 360°视频在VR中广泛应用，但分辨率不足影响视觉质量，且现有VSR技术无法处理其投影失真问题。

Method: 提出S3PO模型，采用循环建模和注意力机制，结合专用特征提取器和新型损失函数处理球形失真。

Result: S3PO在360°视频数据集上优于现有VSR和360°专用超分辨率模型。

Conclusion: S3PO为360°视频超分辨率提供了有效解决方案，并通过消融研究验证了其组件的重要性。
...
TL;DR: 本文提出了一种基于稀疏、松散附着IMU传感器的全身姿态估计新任务，并通过基于Transformer的扩散模型解决了这一问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设IMU传感器紧密附着于人体，但实际场景中这一假设常不成立。本文旨在解决松散附着IMU传感器的姿态估计问题。

Method: 通过模拟现有服装感知运动数据集的IMU记录，开发了基于Transformer的扩散模型，用于合成松散IMU数据并估计姿态。同时，训练中引入服装相关参数以增强模型表达能力。

Result: 实验表明，所提出的扩散方法在模拟和合成数据上训练后，在定量和定性上均优于现有方法。

Conclusion: 该方法为松散IMU传感器的姿态估计开辟了新方向，具有未来研究的潜力。
...
TL;DR: 提出了一种基于扩散模型的单次人脸素描合成方法，通过优化文本指令实现高效生成，并引入新数据集OS-Sketch进行评测。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法依赖大量训练数据、数据稀缺和高人力成本的问题。

Method: 基于扩散模型，通过优化文本指令实现单次训练，并引入OS-Sketch数据集进行评测。

Result: 实验表明，该方法在单次训练下能生成高质量且一致的素描，优于其他方法。

Conclusion: 该方法在便捷性和适用性上具有优势，数据集将公开。
...
TL;DR: Nabla-R2D3是一种基于强化学习的3D扩散模型对齐框架，利用2D奖励信号高效生成高质量3D内容。
...
Conclusion: Nabla-R2D3为3D生成模型提供了一种高效对齐方法，显著提升了生成内容的质量和逼真度。
...
### [155] [An Empirical Study of Bugs in Data Visualization Libraries](https://arxiv.org/abs/2506.15084)
*Weiqi Lu,Yongqiang Tian,Xiaohan Zhong,Haoyang Ma,Zhenyang Xu,Shing-Chi Cheung,Chengnian Sun*

Main category: cs.SE

TL;DR: 该研究首次全面分析了数据可视化库中的错误，发现不正确/不准确的绘图普遍存在，并提出了触发这些错误的八个关键步骤和两个测试预言。


<details>
  <summary>Details</summary>
Motivation: 数据可视化库中的错误可能误导用户，影响决策，因此需要深入理解这些错误的特性以改进库的质量。

Method: 研究分析了来自五个广泛使用的数据可视化库的564个错误，系统性地研究了其症状和根本原因，并提出了分类法。

Result: 研究发现不正确/不准确的绘图是主要问题，图形计算错误是主要原因，同时探索了视觉语言模型在检测错误中的可行性。

Conclusion: 研究为数据可视化库的自动化测试提供了新思路，并指出视觉语言模型在检测错误中的局限性。
...
### [156] [An accurate and revised version of optical character recognition-based speech synthesis using LabVIEW](https://arxiv.org/abs/2506.15029)
...
### [157] [SonicVerse: Multi-Task Learning for Music Feature-Informed Captioning](https://arxiv.org/abs/2506.15154)
...
Conclusion: SonicVerse通过多任务和投影架构，成功生成了丰富的音乐描述，并支持长音乐片段的详细描述生成。
...
### [158] [pycnet-audio: A Python package to support bioacoustics data processing](https://arxiv.org/abs/2506.14864)
...
Method: 利用PNW-Cnet模型扩展功能，检测约80种森林野生动物声音及环境噪声。

Result: 开发了pycnet-audio工具，支持高效处理声学数据。

Conclusion: pycnet-audio为声学监测提供了实用的自动化解决方案。
...
### [159] [Cost-Efficient Serving of LLM Agents via Test-Time Plan Caching](https://arxiv.org/abs/2506.14852)
*Qizheng Zhang,Michael Wornow,Kunle Olukotun*

Main category: cs.DC

TL;DR: 提出了一种名为agentic plan caching的新方法，通过提取、存储和重用结构化计划模板来降低LLM代理应用的成本，平均减少46.62%的成本。
...
Method: 从已完成代理执行中提取计划模板，使用关键词匹配新请求，并通过轻量模型将模板适配到任务特定计划中。

Result: 在多个实际代理应用中，系统平均减少46.62%的成本，同时保持性能。

Conclusion: agentic plan caching为LLM代理应用提供了一种高效且与现有基础设施兼容的解决方案。
